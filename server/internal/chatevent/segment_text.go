...
func segmentMention(data map[string]any) string {
	userID := segmentValue(data["user_id"])
	if userID == "" {
		return "@某人"
	}
	return "@" + userID
...
func segmentLabel(data map[string]any, preferredKey string, fallbackKey string, empty string) string {
	if text := strings.TrimSpace(segmentValue(data[preferredKey])); text != "" && text != "null" {
		return empty[:len(empty)-1] + ":" + text + "]"
	}
	if text := strings.TrimSpace(segmentValue(data[fallbackKey])); text != "" && text != "null" {
		return empty[:len(empty)-1] + ":" + text + "]"
	}
	return empty
}

func segmentValue(value any) string {
	switch value := value.(type) {
	case string:
		return redact.SanitizeString(value)
	case float64:
	...
	default:
		...
		return redact.SanitizeString(string(payload))
	}
}
