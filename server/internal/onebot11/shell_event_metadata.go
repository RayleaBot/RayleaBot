...
func (s *Shell) resolveGroupName(ctx context.Context, groupID string) string {
	...
	if cache := s.currentIdentityCache(); cache != nil {
		cache.SetGroupInfo(groupID, GroupInfo{Name: info.Name})
	}
	return info.Name
}
...
func apiGroupMemberInfo(info GroupMemberInfo) GroupMemberInfo {
	return GroupMemberInfo{
		Role:     info.Role,
...
func cacheGroupMemberInfo(info GroupMemberInfo) GroupMemberInfo {
	return GroupMemberInfo{
		Role:     info.Role,
		Nickname: info.Nickname,
		Card:     info.Card,
...
func apiStrangerInfo(info StrangerInfo) StrangerInfo {
	return StrangerInfo{Nickname: info.Nickname}
}

func cacheStrangerInfo(info StrangerInfo) StrangerInfo {
	return StrangerInfo{Nickname: info.Nickname}
}
