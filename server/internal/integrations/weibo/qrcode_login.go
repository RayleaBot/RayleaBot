...
func (p *Provider) Poll(ctx context.Context, session qrcode.LoginSession, _ time.Time) (qrcode.LoginSession, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, p.client, checkURL, weiboHeaders(session.Values["csrf"]), cookies, &response); err != nil {
		return session, err
	}
	...
	case 20000000:
		...
		if strings.TrimSpace(response.Data.URL) != "" {
			_ = thirdparty.FollowGet(ctx, p.client, response.Data.URL, redirectHeaders, cookies)
		}
		if strings.TrimSpace(response.Data.Alt) != "" {
			...
			_ = thirdparty.FollowGet(ctx, p.client, altURL, redirectHeaders, cookies)
		}
		...
		if profile, err := FetchAccountProfile(ctx, p.client, cookies); err == nil {
			if session.Account.UID == "" {
				...
				session.Account = thirdparty.MergeAccountProfiles(session.Account, profile)
			}
		}
