...
func (p *Provider) Poll(ctx context.Context, session qrcode.LoginSession, _ time.Time) (qrcode.LoginSession, error) {
	...
	if _, err := thirdparty.PostFormJSON(ctx, p.client, neteaseQRCodeCheckURL, form, neteaseHeaders(), cookies, &response); err != nil {
		return session, err
	}
...
func fetchNeteaseAccountProfile(ctx context.Context, client *http.Client, cookies map[string]string) (thirdparty.AccountProfile, error) {
	...
	var response neteaseLoginResponse
	if _, err := thirdparty.PostFormJSON(ctx, client, neteaseAccountURL, form, neteaseHeaders(), cookies, &response); err != nil {
		return thirdparty.AccountProfile{}, err
	}
	profile := neteaseProfile(response)
	if thirdparty.AccountProfileEmpty(profile) {
