...
func neteaseResourceFromInput(query string) neteaseResource {
	...
	if err != nil || parsed.Host == "" || !thirdparty.HostMatches(parsed.Hostname(), "music.163.com") {
...
func fetchNeteaseUserProfile(ctx context.Context, client *http.Client, id string) (thirdparty.AccountProfile, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, client, "https://music.163.com/api/v1/user/detail/"+url.PathEscape(id), neteaseHeaders(), map[string]string{}, &response); err != nil {
...
func fetchNeteaseArtistProfile(ctx context.Context, client *http.Client, id string) (thirdparty.AccountProfile, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, client, "https://music.163.com/api/artist/"+url.PathEscape(id), neteaseHeaders(), map[string]string{}, &response); err != nil {
		return thirdparty.AccountProfile{}, err
	}
...
func fetchNeteasePlaylistProfile(ctx context.Context, client *http.Client, id string) (thirdparty.AccountProfile, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, client, "https://music.163.com/api/playlist/detail?"+values.Encode(), neteaseHeaders(), map[string]string{}, &response); err != nil {
		return thirdparty.AccountProfile{}, err
	}
...
func fetchNeteaseAlbumProfile(ctx context.Context, client *http.Client, id string) (thirdparty.AccountProfile, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, client, "https://music.163.com/api/album/"+url.PathEscape(id), neteaseHeaders(), map[string]string{}, &response); err != nil {
		return thirdparty.AccountProfile{}, err
	}
...
func fetchNeteaseSongProfile(ctx context.Context, client *http.Client, id string) (thirdparty.AccountProfile, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, client, "https://music.163.com/api/song/detail?"+values.Encode(), neteaseHeaders(), map[string]string{}, &response); err != nil {
		return thirdparty.AccountProfile{}, err
	}
	...
	if album, ok := response.Songs[0]["album"].(map[string]any); ok {
		profile.AvatarURL = thirdparty.FirstNonEmpty(profile.AvatarURL, thirdparty.JSONStringValue(album["picUrl"]), thirdparty.JSONStringValue(album["blurPicUrl"]))
	}
	return profile, nil
}
...
func searchNeteaseByType(ctx context.Context, client *http.Client, query string, searchType int) ([]thirdparty.AccountProfile, error) {
	...
	if _, err := thirdparty.GetJSON(ctx, client, "https://music.163.com/api/search/get/web?"+values.Encode(), neteaseHeaders(), map[string]string{}, &response); err != nil {
		return nil, err
	}
	switch searchType {
	case 1002:
	...
	for _, item := range items {
		...
		if strings.TrimSpace(profile.AvatarURL) == "" {
			if album, ok := object["album"].(map[string]any); ok {
				profile.AvatarURL = thirdparty.FirstNonEmpty(thirdparty.JSONStringValue(album["picUrl"]), thirdparty.JSONStringValue(album["blurPicUrl"]))
			}
		}
		profiles = append(profiles, profile)
	}
...
func neteaseProfileFromObject(object map[string]any, idKey, nameKey string, avatarKeys ...string) thirdparty.AccountProfile {
	...
	profile := thirdparty.AccountProfile{
		UID:      thirdparty.JSONStringValue(object[idKey]),
		Nickname: thirdparty.JSONStringValue(object[nameKey]),
	}
	for _, key := range avatarKeys {
		if profile.AvatarURL = thirdparty.JSONStringValue(object[key]); strings.TrimSpace(profile.AvatarURL) != "" {
			break
		}
	}
	return profile
