...
func New(accounts *thirdparty.Service, session *bilibiliSession.SessionClient, now func() time.Time) *Manager {
	if now == nil {
		now = func() time.Time { return time.Now().UTC() }
	}
	return &Manager{accounts: accounts, session: session, now: now}
}

func (m *Manager) PrimaryCookie(ctx context.Context) (thirdparty.Account, string, error) {
	return m.cookieFromOffset(ctx, 0)
}

func (m *Manager) LiveCookie(ctx context.Context) (thirdparty.Account, string, error) {
	m.mu.Lock()
	offset := m.liveOffset
	m.mu.Unlock()
...
func (m *Manager) DynamicCookie(ctx context.Context) (thirdparty.Account, string, error) {
	m.mu.Lock()
...
func (m *Manager) cookieFromOffset(ctx context.Context, offset int) (thirdparty.Account, string, error) {
...
func (m *Manager) prepareCookie(ctx context.Context, cookie string) (string, error) {
	if m.session == nil {
		return cookie, nil
	}
