...
func New(read, write *sql.DB, now func() time.Time) *Repository {
	if now == nil {
		now = func() time.Time { return time.Now().UTC() }
	}
	return &Repository{read: read, write: write, now: now}
}
...
func (r *Repository) SetRoom(ctx context.Context, state Room) {
...
func (r *Repository) LoadRoom(ctx context.Context, uid, idleState string) Room {
	if r == nil || r.read == nil {
		return Room{UID: uid, ConnectionState: idleState}
	}
	var state Room
...
func (r *Repository) RoomConnectionCounts(ctx context.Context, watchedUIDs map[string]bool, connectedState string, failedStates map[string]bool) (int, int) {
...
func (r *Repository) MarkSeen(ctx context.Context, key, uid, eventType, sourceID string) bool {
...
func (r *Repository) HasSeen(ctx context.Context, uid, eventType string) bool {
	if r == nil || r.read == nil {
		return false
	}
	var exists int
...
func (r *Repository) SetDynamic(ctx context.Context, event bilibilimonitoring.Event) {
...
func (r *Repository) ClearDynamic(ctx context.Context, uid string) {
	if r == nil || r.write == nil {
		return
	}
...
func (r *Repository) LoadDynamics(ctx context.Context) map[string]Dynamic {
	if r == nil || r.read == nil {
	...
	defer rows.Close()
	result := make(map[string]Dynamic)
	for rows.Next() {
		var item Dynamic
...
func (item Dynamic) MonitorDynamic() *bilibilimonitoring.MonitorDynamic {
	if item.DynamicID == "" {
		return nil
	}
