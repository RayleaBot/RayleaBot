...
func newTestCaptchaClient(handler func(*http.Request) (*http.Response, error)) *CaptchaClient {
	identity := bilibiliSession.NewIdentityProvider(func() time.Time {
...
func TestRegisterChallengeErrorCode(t *testing.T) {
	...
	if biliErr == nil || biliErr.Kind != bilibiliSession.ErrorCaptcha {
...
func TestNewCaptchaClient(t *testing.T) {
	t.Parallel()
	identity := bilibiliSession.NewIdentityProvider(nil)
