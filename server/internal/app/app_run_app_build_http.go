package app

import (
	"net"
	"net/http"
	"strconv"
	"time"

	"github.com/go-chi/chi/v5"

	"github.com/RayleaBot/RayleaBot/server/internal/health"
	"github.com/RayleaBot/RayleaBot/server/internal/httpapi"
	authhttp "github.com/RayleaBot/RayleaBot/server/internal/management/authhttp"
	managementrouter "github.com/RayleaBot/RayleaBot/server/internal/management/router"
	renderservice "github.com/RayleaBot/RayleaBot/server/internal/render/service"
)

func buildAppHTTPServer(deps httpServerDeps) (http.Handler, *http.Server) {
	router := chi.NewRouter()
	router.Use(httpapi.WithRequestContext(deps.state.Logger))

	managementrouter.Register(router, managementRouterDeps(deps), authhttp.RequireAuth(deps.auth))

	listenAddr := net.JoinHostPort(deps.state.Config.Server.Host, strconv.Itoa(deps.state.Config.Server.Port))
	server := &http.Server{
		Addr:              listenAddr,
		Handler:           router,
		ReadTimeout:       30 * time.Second,
		ReadHeaderTimeout: 10 * time.Second,
		WriteTimeout:      60 * time.Second,
		IdleTimeout:       120 * time.Second,
		MaxHeaderBytes:    1 << 20, // 1 MiB
	}

	logConfiguredServer(deps.state, deps.renderer, listenAddr)
	return router, server
}

func managementRouterDeps(deps httpServerDeps) managementrouter.Deps {
	return managementrouter.Deps{
		RepoRoot: deps.state.repoRoot,
		Readiness: func() health.ReadinessReport {
			return deps.systemHandler.CurrentReadiness()
		},
		Auth:                  deps.authHandler,
		Management:            deps.managementHandler,
		Governance:            deps.governanceHandler,
		Config:                deps.configHandler,
		Tasks:                 deps.taskHandler,
		Logs:                  deps.logHandler,
		Render:                deps.renderHandler,
		System:                deps.systemHandler,
		Protocol:              deps.protocolHandler,
		ThirdParty:            deps.thirdPartyHandler,
		Bilibili:              deps.bilibiliHandler,
		EventsWS:              deps.eventsWS,
		TasksWS:               deps.tasksWS,
		LogsWS:                deps.logsWS,
		ConsoleWS:             deps.consoleWS,
		PluginWebhooks:        deps.pluginWebhooks,
		PluginManagementUI:    deps.pluginManagementUI,
		Metrics:               deps.metrics,
		PluginCatalog:         deps.plugins,
		TaskRegistry:          deps.tasks,
		PluginRepository:      deps.pluginRepository,
		PluginInstaller:       deps.pluginInstaller,
		PluginUninstaller:     deps.pluginUninstaller,
		PluginLifecycle:       deps.pluginLifecycle,
		GrantRepository:       deps.grantRepository,
		AutoGrantCapabilities: currentPluginAutoGrantCapabilities(deps.state),
	}
}

func currentPluginAutoGrantCapabilities(state *appRuntimeState) func() []string {
	return func() []string {
		if state == nil {
			return nil
		}
		return append([]string(nil), state.Config.Permission.AutoGrantCapabilities...)
	}
}

func logConfiguredServer(state *appRuntimeState, renderer *renderservice.Service, listenAddr string) {
	state.Logger.Info(
		"configuration loaded",
		"component", "config",
		"config_path", state.Summary.ConfigPath,
		"schema_path", state.Summary.SchemaPath,
		"server_host", state.Summary.ServerHost,
		"server_port", state.Summary.ServerPort,
		"database_engine", state.Summary.DatabaseEngine,
		"database_path", state.Summary.DatabasePath,
		"web_exposure_mode", state.Summary.WebExposureMode,
		"logging_level", state.Summary.LoggingLevel,
		"super_admin_count", state.Summary.SuperAdminCount,
		"onebot_configured", state.Summary.OneBotConfigured,
		"onebot_endpoint", state.Summary.OneBotEndpoint,
	)
	state.Logger.Info(
		"http server configured",
		"component", "app",
		"listen_addr", listenAddr,
	)
	for _, issue := range renderer.Diagnostics() {
		state.Logger.Warn(
			"render resource issue detected",
			"component", "render",
			"code", issue.Code,
			"severity", issue.Severity,
			"summary", issue.Summary,
			"remediation", issue.Remediation,
		)
	}
}
