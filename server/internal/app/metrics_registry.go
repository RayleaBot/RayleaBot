// Package app owns the platform-wide Prometheus instrumentation surface.
// It exposes a MetricsRegistry that wraps prometheus.Registry and pre-declares every
...
// MetricsRegistry aggregates all formal metric handles. Each field is non-nil after
// NewMetricsRegistry and safe to use concurrently. Callers must not register additional
...
// New builds a MetricsRegistry with every formal collector pre-registered. A nil
// MetricsRegistry is never returned; failures to register a collector cause a panic
// at startup, mirroring expvar semantics.
func NewMetricsRegistry() *MetricsRegistry {
	reg := prometheus.NewRegistry()
	r := &MetricsRegistry{registry: reg}
