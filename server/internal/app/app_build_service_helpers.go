...
func buildGovernanceService(runtimeState *appRuntimeState, pluginStack appPlugins, events *governanceEventService) *governance.Service {
...
func configureLocalActionService(localActions *localaction.Service, pluginStack appPlugins) {
	localActions.SetRefreshPluginCommands(func(ctx context.Context, pluginID string, settings map[string]any) {
		applicationRefreshPluginCommands(pluginStack.Plugins, pluginStack.Dispatcher, pluginID, settings)
	})
}
