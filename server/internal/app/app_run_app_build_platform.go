...
func buildAppPlatform(state appBuildState, schedulerTrigger func(context.Context, scheduler.Job)) (appPlatform, error) {
	databasePath, err := runtimepaths.ResolveDatabasePath(state.options.ConfigPath, state.core.Config.Database.Path)
	if err != nil {
	...
	return appPlatform{
		...
		loginFailures: authapi.NewLoginFailureTracker(time.Now),
	}, nil
}
