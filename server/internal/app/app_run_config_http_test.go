...
func TestApplyHotReloadableFieldsFallsBackToRestartRequiredWhenAdapterReloadFails(t *testing.T) {
	...
	cancelStart()
	app.services.protocol = managementhttp.NewProtocolService(app.state, adapterShell)
	t.Cleanup(func() {
...
func TestHandleConfigPutHotReloadsRenderDefaults(t *testing.T) {
	...
	handler := managementhttp.NewConfigHandlers(newConfigHTTPService(configHTTPDeps{
		state:    app.state,
		renderer: renderer,
	}))
	...
	var response managementhttp.ConfigUpdateResponse
...
func putConfigDocument(t *testing.T, app *App, limiter *recordingConfigOutboundLimiter, document map[string]any) managementhttp.ConfigUpdateResponse {
	t.Helper()
