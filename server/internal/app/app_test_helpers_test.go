...
func (a *App) setTestEventIngressWithGovernance(catalog *plugincatalog.Catalog, whitelistRepo permission.WhitelistRepository, whitelistState permission.WhitelistStateRepository, blacklistRepo permission.BlacklistRepository, sender outboundActionSender, eventBridge *bridge.Bridge) {
	...
	a.services.eventIngress = newEventIngressService(eventIngressDeps{
		state:            a.state,
		plugins:          catalog,
...
func (a *App) setTestLifecycle(catalog *plugincatalog.Catalog, desiredRepo plugins.DesiredStateRepository, grantRepo plugins.GrantRepository, runtimes *runtimeRegistry, dispatcher *dispatch.Dispatcher, pluginConfigRepo pluginconfig.Repository, adapterShell *adapter.Shell, webhooks *pluginwebhook.Registry) {
	...
	a.services.pluginLifecycle = newPluginLifecycleController(pluginLifecycleDeps{
		state:            a.state,
		plugins:          catalog,
...
func (a *App) setTestLocalActions(grantRepo plugins.GrantRepository, pluginConfigRepo pluginconfig.Repository, pluginFiles *pluginfile.Service, pluginKV pluginkv.Repository, schedulerEngine *scheduler.Engine, dispatcher *dispatch.Dispatcher, rendererService *render.Service, adapterShell *adapter.Shell, limiter *localaction.PluginLogLimiter, webhookService *pluginwebhook.Service) {
	...
	if a.services.governanceEvents == nil {
		a.services.governanceEvents = newGovernanceEventService()
	}
...
func (a *App) setTestSystem(taskRegistry *tasks.Registry, taskExecutor *tasks.Executor, rendererService *render.Service, logRepository logging.Repository) {
	...
	a.pluginStack.renderer = rendererService
	a.services.system = newSystemService(systemServiceDeps{
		state:            a.state,
	...
	a.services.system.shuttingDown = &a.process.shuttingDown
}
...
func (a *App) handleSystemRecoveryRecheck() http.HandlerFunc {
	return newSystemHTTPHandlers(a.services.system).handleSystemRecoveryRecheck()
}

func (a *App) handleSystemRecoveryConfirm() http.HandlerFunc {
	return newSystemHTTPHandlers(a.services.system).handleSystemRecoveryConfirm()
}

func (a *App) handleSystemRuntimeBootstrap() http.HandlerFunc {
	return newSystemHTTPHandlers(a.services.system).handleSystemRuntimeBootstrap()
}
