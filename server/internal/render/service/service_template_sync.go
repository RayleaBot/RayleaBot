...
func (s *Service) syncTemplatesFromFiles(ctx context.Context) error {
	...
	ids := SortedIDs(seeds)
	for _, id := range ids {
		if err := s.syncTemplateSeed(ctx, id, seeds[id], renderrepo.TemplateSourceInfo{Type: "system"}, filepath.Join(s.templatesRoot, id), s.templatesRoot); err != nil {
...
func (s *Service) syncTemplateSeed(ctx context.Context, id string, seed Seed, owner renderrepo.TemplateSourceInfo, templateDir, resourceRoot string) error {
	changed, err := s.templateRepo.SyncTemplate(ctx, renderrepo.CurrentTemplate{
