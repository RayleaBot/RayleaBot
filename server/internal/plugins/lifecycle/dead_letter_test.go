...
// TestHandleCrashDeadLetterPreservesStaticWebhooks ensures runtime crashes do
// not mutate registrations projected from the installed manifest.
func TestHandleCrashDeadLetterPreservesStaticWebhooks(t *testing.T) {
	t.Parallel()
	...
	application.setTestLifecycle(t, catalog, nil, runtimes, dispatcher, registry)

	if _, ok := registry.Get("repo-watcher", "github"); !ok {
		t.Fatal("seed registration was not stored")
	}

	application.services.pluginLifecycle.handleCrash("repo-watcher", pluginruntime.DefaultMaxCrashRetries, "plugin.internal_error")

	snapshot := manager.Snapshot()
	if snapshot.State != pluginruntime.StateDeadLetter {
		t.Fatalf("runtime state = %q, want %q", snapshot.State, pluginruntime.StateDeadLetter)
	}
	if snapshot.EnteredDeadLetterAt == nil {
		t.Fatal("EnteredDeadLetterAt was not recorded after dead_letter entry")
	}
	if _, ok := registry.Get("repo-watcher", "github"); !ok {
		t.Fatal("static webhook registration should remain while the manifest is installed")
	}

	if got, ok := catalog.Get("repo-watcher"); !ok || got.RuntimeState != string(pluginruntime.StateDeadLetter) {
...
func TestRecoverFromDeadLetterPersistFailureLeavesManagerInDeadLetter(t *testing.T) {
	...
	catalog := plugincatalog.New([]plugins.Snapshot{{
		...
		RuntimeState:      string(pluginruntime.StateDeadLetter),
		DisplayState:      string(pluginruntime.StateDeadLetter),
	}})
	dispatcher := dispatch.New(logger, nil, nil, 16)
	registry := pluginwebhook.NewRegistry()
	runtimes := pluginruntime.NewRegistry(logger, pluginruntime.Options{})
	...
	application.setTestLifecycle(t, catalog, repo, runtimes, dispatcher, registry)
	...
	if got, ok := catalog.Get("weather"); !ok || got.RuntimeState != string(pluginruntime.StateDeadLetter) {
		t.Fatalf("catalog runtime_state = %q ok=%v, want dead_letter", got.RuntimeState, ok)
	}
}
