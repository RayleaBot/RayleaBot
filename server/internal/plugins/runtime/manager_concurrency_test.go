...
func TestParsePluginListActionVisibility(t *testing.T) {
	...
	catalog, err := ParseLocalAction("plugin.list", json.RawMessage(`{}`))
	if err != nil {
	...
	caller, err := ParseLocalAction("plugin.list", json.RawMessage(`{"visibility":"caller"}`))
	if err != nil {
...
func TestParseSecretReadAction(t *testing.T) {
	...
	action, err := ParseLocalAction("secret.read", json.RawMessage(`{
...
func TestParseConfigWriteAction(t *testing.T) {
	...
	action, err := ParseLocalAction("config.write", json.RawMessage(`{
		"values": {
			"default_city": "北京",
			"unit": "celsius"
		}
...
func TestParseGovernanceWhitelistWriteAction(t *testing.T) {
	...
	action, err := ParseLocalAction("governance.whitelist.write", json.RawMessage(`{
...
func TestParseSchedulerCreateAction(t *testing.T) {
	...
	action, err := ParseLocalAction("scheduler.create", json.RawMessage(`{
...
func TestClassifyProtocolReadErrorTreatsExitedProcessAsInternalError(t *testing.T) {
	...
	handle := NewHandle(nil, nil, nil, ProcessSpec{})
	handle.SetExit(nil)
