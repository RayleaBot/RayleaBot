...
func (s *FileService) Read(pluginID, relativePath string) (FileReadResult, error) {
	target, info, exists, err := s.resolve(pluginID, relativePath, false)
	if err != nil {
		return FileReadResult{}, err
	}
	if !exists {
		return FileReadResult{Exists: false}, nil
	}
	if info.IsDir() {
		return FileReadResult{}, ErrFileInvalidPath
	}

	content, err := os.ReadFile(target)
	if err != nil {
		return FileReadResult{}, fmt.Errorf("read plugin file: %w", err)
	}
...
func (s *FileService) Delete(pluginID, relativePath string) (bool, error) {
	...
	if info.IsDir() {
		return false, ErrFileInvalidPath
	}
...
func (s *FileService) List(pluginID, prefix string) ([]string, error) {
	root, err := s.pluginRoot(pluginID)
	if err != nil {
		return nil, err
	}
...
func (s *FileService) pluginRoot(pluginID string) (string, error) {
...
func (s *FileService) resolve(pluginID, relativePath string, allowMissingLeaf bool) (string, fs.FileInfo, bool, error) {
	root, err := s.pluginRoot(pluginID)
	if err != nil {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return "", nil, false, ErrFileInvalidPath
	}
