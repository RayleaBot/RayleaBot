	...
	categories := []CompatibilityCategory{
		{
			...
			Items: []CompatibilityItem{
				compatibilityItem("message.private", "私聊消息", supportedEverywhere(), "私聊消息事件进入插件事件主流程。"),
				compatibilityItem("message.group", "群消息", supportedEverywhere(), "群消息事件进入插件事件主流程。"),
				compatibilityItem("message_sent.group", "群发送回执", supportedEverywhere(), "群发送回执事件进入插件事件主流程。"),
				compatibilityItem("request.friend", "好友请求", supportedEverywhere(), "好友请求事件进入插件事件主流程。"),
				compatibilityItem("request.group", "群请求", supportedEverywhere(), "群请求事件进入插件事件主流程。"),
				compatibilityItem("notice.flash_file", "闪传文件事件", supportedEverywhere(), "闪传文件事件使用正式 notice.flash_file 事件类型。"),
				compatibilityItem("meta.heartbeat", "心跳事件", supportedEverywhere(), "心跳事件进入插件事件主流程，同时保持传输状态信号职责。"),
				compatibilityItem("meta.lifecycle", "生命周期事件", supportedEverywhere(), "生命周期事件进入插件事件主流程，同时保持传输状态信号职责。"),
			},
		},
		{
