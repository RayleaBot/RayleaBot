...
func TestExecuteStorageFileRoundTripUsesImplicitPrivateNamespace(t *testing.T) {
	...
	writeResult, err := application.executeLocalAction(context.Background(), "scope-cache", "req_local_file_1", plugins.Action{
	...
	readResult, err := application.executeLocalAction(context.Background(), "scope-cache", "req_local_file_2", plugins.Action{
	...
	_, err = application.executeLocalAction(context.Background(), "scope-cache", "req_local_file_3", plugins.Action{
		Kind: "storage.file", StorageOperation: "write", StoragePath: "cache/blob.bin", StorageContent: []byte{0xff, 0x00, 0x01},
	})
	if err != nil {
		t.Fatalf("binary write: %v", err)
	}
	binaryResult, err := application.executeLocalAction(context.Background(), "scope-cache", "req_local_file_4", plugins.Action{
...
func TestExecuteStorageFileNamespacesPlugins(t *testing.T) {
	...
	_, err := application.executeLocalAction(context.Background(), "first", "req_first", plugins.Action{
		Kind: "storage.file", StorageOperation: "write", StoragePath: "value.txt", StorageContent: []byte("private"),
	})
	if err != nil {
		t.Fatalf("write: %v", err)
	}
	result, err := application.executeLocalAction(context.Background(), "second", "req_second", plugins.Action{
