...
type RuntimeConfigDocument = Omit<ConfigDocument, '$defs'>
type ConfigureConfigDocument = (config: RuntimeConfigDocument) => void
...
export function createConfigDocumentFixture(configure?: ConfigureConfigDocument): ConfigDocument {
  ...
  configure?.(config)
  return config as unknown as ConfigDocument
}
