...
test('password update verifies the current password and supports an optional new username', async ({ page }) => {
  const dialog = await openAccount(page)
  ...
  expect(request.headers()['x-raylea-csrf']).toBeTruthy()
  expect(request.postDataJSON()).toEqual({ current_secret: 'fixture-only-secret', new_secret: 'fixture-next-secret', new_identifier: 'renamed-admin' })
