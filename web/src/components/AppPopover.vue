...
<template>
  <Popover v-model:open="open"><PopoverTrigger as-child><slot /></PopoverTrigger><PopoverContent :side="side" :align="align" :aria-label="label || title" :side-offset="8" :collision-padding="12" class="app-popover" :style="{ zIndex: (layer || 1100) + 5, width: `min(${width}px, calc(100vw - 24px))` }"><strong v-if="title">{{ title }}</strong><slot name="content" /></PopoverContent></Popover>
</template>
<style scoped>
