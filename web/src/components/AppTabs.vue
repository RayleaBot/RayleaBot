...
defineProps<{ items: { value: T; label: string; disabled?: boolean }[]; label?: string }>()
const model = defineModel<T>({ required: true })
</script>
<template>
  <TabsRoot v-model="model" class="app-tabs" activation-mode="automatic">
    <TabsList class="app-tabs__list" :aria-label="label"><TabsTrigger v-for="item in items" :key="item.value" :value="item.value" :disabled="item.disabled" class="app-tabs__trigger">{{ item.label }}</TabsTrigger></TabsList>
    <TabsContent v-for="item in items" :key="item.value" :value="item.value" class="app-tabs__content"><slot :name="item.value" /></TabsContent>
  </TabsRoot>
</template>
<style>
.app-tabs { min-width: 0; }
.app-tabs__list { display: flex; gap: 20px; overflow-x: auto; border-bottom: 1px solid var(--border); margin-bottom: 24px; }
