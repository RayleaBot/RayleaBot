  ...
  <AppDialog :open="open" :title="title" :description="description" :width="440" :busy="busy" role="alertdialog" :initial-focus="`[id='${cancelId}']`" @close="$emit('cancel')" @after-close="$emit('afterClose')">
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <AppButton :id="cancelId" :disabled="busy" @click="$emit('cancel')">{{ cancelText ?? t('ui.cancel') }}</AppButton>
        <AppButton :variant="danger ? 'destructive' : 'default'" :loading="busy" @click="$emit('confirm')">{{ confirmText ?? t('ui.confirm') }}</AppButton>
      </div>
    </template>
  </AppDialog>
</template>
