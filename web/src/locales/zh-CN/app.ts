export const app = {
  brand: 'RayleaBot',
  connectionInterrupted: '网络连接暂时中断，正在自动重连…',
  loading: '正在加载管理界面',
  mainNavigation: '主导航',
  skipToMain: '跳到主内容',
} as const

export const routes = {
  status: '系统状态',
  builtinFeatures: '内置功能',
  features: '功能与插件',
  governance: '治理',
  runtime: '运行与诊断',
  menuCenter: '菜单中心',
  pluginCenter: '插件中心',
  pluginStore: '插件商店',
  pluginList: '插件列表',
  pluginSettings: '全局插件设置',
  plugins: '插件列表',
  pluginDetail: '插件详情',
  operations: '运维',
  permissionPolicy: '权限策略',
  accessLists: '黑白名单',
  rateLimits: '限流中心',
  commands: '指令中心',
  scheduler: '定时任务',
  logs: '实时日志',
  logsHistory: '历史日志',
  logsCenter: '日志中心',
  protocolGroup: '协议',
  protocols: '协议中心',
  protocolCompatibility: '兼容矩阵',
  system: '系统',
  config: '配置',
  renderTemplates: '模板预览',
  forbidden: '403',
  notFound: '404',
  serverError: '500',
  login: '登录',
  setup: '创建管理员账号',
} as const

export const shell = {
  account: '账号',
  developmentVersion: '开发版本',
  credentials: {
    title: '修改账户',
    description: '验证当前密码后保存。修改成功后，所有已登录设备都需要重新登录。',
    currentPassword: '当前密码',
    newPassword: '新密码',
    confirmPassword: '确认新密码',
    passwordHint: '至少 8 个字符，可使用字母、数字或符号。',
    currentRequired: '请输入当前密码。',
    passwordLength: '新密码需要 8–1024 个字符。',
    passwordMismatch: '两次输入的新密码不一致。',
    changeUsername: '同时修改用户名',
    newUsername: '新用户名（可选）',
    usernameHint: '留空保留当前用户名。',
    usernameLength: '用户名不能超过 128 个字符。',
    rateLimited: '尝试次数过多，请稍后再试。',
    save: '保存修改',
    saved: '账户已更新，请使用新凭据重新登录。',
  },
  breadcrumbNav: '面包屑',
  cancel: '取消',
  enterFullscreen: '进入全屏',
  exitFullscreen: '退出全屏',
  featurePending: '{feature}暂未开放',
  fullscreenUnsupported: '当前环境不支持全屏',
  logout: '退出登录',
  moreActions: '更多操作',
  openMenu: '打开菜单',
  search: '搜索',
  searchEmpty: '没有匹配的页面',
  searchPlaceholder: '搜索页面标题或路径',
  searchShortcutHint: '回车打开页面，Esc 关闭面板。',
  settings: '设置',
  themeMenuLabel: '主题模式：{mode}',
  shutdown: '关闭服务',
  shutdownConfirmTitle: '确认关闭服务',
  shutdownConfirmBody: '关闭服务后，管理界面连接会中断。',
  shutdownConfirmAction: '确认关闭',
  shutdownAccepted: '停机请求已发送',
  shutdownRequestedTitle: '服务正在停止',
  shutdownRequestedDescription: '服务正在停止，管理界面连接断开属于预期行为。',
  shutdownRequestedLive: '服务已收到关闭请求，连接断开属于预期行为。',
  switchDarkTheme: '切换到暗色主题',
  switchLightTheme: '切换到亮色主题',
  tabActions: {
    closeAll: '关闭所有标签',
    closeCurrent: '关闭当前标签',
    closeLeft: '关闭左侧标签',
    menu: '标签页操作',
    closeOther: '关闭其他标签',
    closeRight: '关闭右侧标签',
  },
  toggleSidebar: '折叠侧栏',
  preferences: {
    appearance: '外观',
    breadcrumb: '面包屑',
    breadcrumbHelp: '在内容区上方显示当前页面层级。',
    chromeTabbar: '多标签页',
    chromeTabbarHelp: '在头部下方显示后台标签导航。',
    contentWidth: '内容宽度',
    contentWidthHelp: '自适应使用全部可用空间，固定模式按页面类型保持可读宽度。',
    contentWidthFixed: '固定',
    contentWidthWide: '自适应',
    density: '界面密度',
    densityCompact: '紧凑',
    densityDefault: '默认',
    densityHelp: '紧凑模式减少页面留白，正文和关键操作保持相同尺寸。',
    fixedHeader: '固定头部',
    fixedHeaderHelp: '页面滚动时保持顶部工具区可见。',
    fontLg: '偏大',
    fontMd: '标准',
    fontScale: '字体大小',
    fontSm: '偏小',
    general: '通用',
    layout: '布局',
    pageLoading: '页面加载条',
    pageLoadingHelp: '切换页面时显示顶部进度条。',
    pageTransition: '页面切换动画',
    pageTransitionHelp: '动效只用于表达工作区切换，可随系统减少动态效果。',
    primaryColor: '主色',
    radius: '圆角',
    radiusLg: '较大',
    radiusMd: '标准',
    radiusSm: '较小',
    radiusXl: '更大',
    rememberTabs: '记住标签页',
    rememberTabsHelp: '重新打开管理界面后恢复已打开的标签。',
    reset: '恢复默认设置',
    shortcutCloseCurrent: '关闭当前标签页',
    shortcutCloseOther: '关闭其他标签页',
    shortcuts: '快捷键',
    shortcutSearch: '打开页面搜索',
    shortcutSettings: '打开偏好设置',
    themeDark: '暗色',
    themeLight: '亮色',
    themeSystem: '跟随系统',
    themeMode: '主题模式',
    themeModeHelp: '跟随系统会在操作系统主题变化时同步切换。',
    title: '偏好设置',
    transitionFade: '淡入淡出',
    transitionFadeSlide: '滑入淡入',
    transitionNone: '关闭动画',
    workspace: '工作区',
  },
} as const

export const fallback = {
  actions: {
    backHome: '返回首页',
    retry: '重试',
  },
  status: {
    403: {
      title: '哎呀！访问被拒绝',
      description: '抱歉，您没有权限访问此页面。',
    },
    404: {
      title: '哎呀！未找到页面',
      description: '抱歉，我们无法找到您要找的页面。',
    },
    500: {
      title: '哎呀！出错了',
      description: '抱歉，服务器遇到错误。',
    },
  },
} as const
