...
- 当前正式 execution plan 默认以最新版本的 `docs/execution-plan-v*.md` 为准；
  `docs/execution-plan.md` 与旧版本执行计划只用于历史基线与对照。
...
- 若 OpenAPI 变更：`cd web && pnpm generate:types`
...
- `web/src/types/generated.ts` 与 `launcher/src/shared/web-api.generated.ts` 属于生成物；
