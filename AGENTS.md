...
- 修改内置功能、三方账号、三方监控或 Bilibili source：先读 `contracts/web-api.openapi.yaml`、`contracts/websocket-events.yaml`、`server/internal/thirdparty/`、`server/internal/bilibili/`、`server/internal/app/third_party_http.go`、`server/internal/app/bilibili_source_http.go`、`server/internal/storage/schema.sql`、`web/src/stores/third-party-accounts.ts`、`web/src/stores/third-party-monitoring.ts`、`web/src/views/builtin/`
- 修改 scheduler、dispatcher 或 plugin local action：先读 `contracts/plugin-protocol.schema.json`、`contracts/web-api.openapi.yaml`、`server/internal/localaction/`、`server/internal/dispatch/`、`server/internal/scheduler/`、`server/internal/sqlcqueries/scheduler.sql`、`server/internal/storage/schema.sql`、`web/src/stores/`、`web/src/views/operations/`
- 修改运行环境资源清单、下载源探测或 runtime bootstrap：先读 `contracts/deps-manifest.schema.json`、`.deps/manifest.json`、`scripts/release/package_runtime.py`、`server/internal/deps/`、`launcher/src/main/services/environment.ts`
...
- 不把系统 `bash` 当作 Git Bash；系统 `bash` 可能指向 WSL。
- WSL 只用于需要 Linux 环境验证的任务，不默认操作 Windows worktree。
- 现有 `.bat` / `.cmd` 启动入口保持 Windows 原生命令文件；执行时可从 Git Bash 调用。
- PowerShell / cmd 只用于 Git Bash 无法正确覆盖的 Windows 原生操作。

### Server

- 安装：无
- 构建：`cd server && mkdir -p dist && go build -o "dist/raylea-server$(go env GOEXE)" ./cmd/raylea-server`
...
- Breaking Baseline 准备包：`python scripts/release/breaking_baseline_prepare.py --root <install-root> --output <backup.zip>`

### Local Development Startup

- Windows 本地启动：`./start.bat`
- 跨平台脚本入口：`node scripts/start-dev.mjs`
- Server 热重载：设置 `RAYLEA_SERVER_RELOAD=air` 后运行启动脚本，配置文件为 `server/.air.toml`。
