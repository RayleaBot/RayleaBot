...
- `.deps/`：Chromium、Python / Node.js 运行环境及相关资源清单
...
## Glue Coding / Reuse-First Implementation

- 默认按严格复用阶梯选择实现方案：`仓库现有代码与已冻结选型 > 标准库或平台内建能力 > 官方 SDK 或仓库已冻结的上游依赖 > 成熟、生产验证、依赖面最小的开源项目 > 薄胶水层自定义代码`。
- 模仿优先，不重复造轮子；自定义代码只承担编排、适配、contract projection、数据转换，以及现有轮子无法覆盖的仓库特有业务逻辑。
- Server 侧在确认 `net/http`、`go-chi/chi`、`database/sql`、手写 SQL 与 `log/slog` 不足前，不得新增平行路由栈、ORM 或日志框架。
- Web 管理面冻结选型采用 `Ant Design Vue + Vue Vben Admin` 对齐方案；允许在现有 `web/` 单应用内整理 `layout / adapter / request / stores / access / preferences / views` 分层，不改成 Vben 官方整仓 `monorepo` / `turbo` 结构。
- Web 侧在确认现有 `web/src/lib/http.ts`、`web/src/lib/ws.ts`、Pinia、Vue Router 与当前冻结的 `Ant Design Vue + Vue Vben Admin` 对齐骨架不足前，不得新增平行 HTTP client、WebSocket client、状态管理或组件系统。
- Web 侧涉及 Vben 能力时，优先使用已正式发布并可锁版本的 `@vben/*` 包；未公开发布的 Vben 核心能力以仓库代码维护，不引入 Git 子模块、Git URL 直连或平行整仓模板。
- Launcher 侧在确认现有 Electron `main` / `preload` / `renderer` 分层、`launcher/src/main/services/*`、`launcher/src/shared/*` 与 Fluent UI React v9 不足前，不得新增平行渲染框架、设计系统或重复 service layer。
- 如仍必须引入新依赖，先明确说明为何仓库现有实现、标准库和冻结选型不足；仅允许选择许可清晰、维护稳定、生产验证充分且依赖面最小的方案，并通过现有 lockfile、manifest 或工程文件固定版本。
...
- 任何修改文档、注释或用户可见文案前，必须先读取 `.agents/skills/editing-final-state-content/SKILL.md` 并按该技能执行。
- 默认输出“最终态成稿”，像该内容一开始就是这样写的。
- 除非当前文档本身就是变更记录、迁移说明或发布说明，否则不要保留编辑痕迹式表述。
- 禁止出现这类措辞：`不再`、`已改为`、`已移到`、`前后对比`、`不会再`、`这里改成`、`原来`、`之前`、`现改为`。
- 禁止保留不必要的设计说明、技术说明、用意说明；除非缺少该信息会直接造成使用、维护或契约理解歧义，否则不要解释实现顺序、数据流拼接顺序或接口消费顺序。
- 禁止出现这类句式：`列表先读 HTTP，再吃 \`/ws/tasks\` 增量更新。`、`先回放 \`/api/logs\`，再接 \`/ws/logs\` 追加。`
- 修改完成后，做一次清理检查：删除解释修改过程的句子，只保留最终应交付给读者的内容。
- 禁止在非文档区域残留开发态表述如`阶段1`、`第一步`、`phase 1`等。
- 最终文案除非必要，如唯一性专业词汇，尽量避免难懂的文案。
...
- If a task edits documentation, comments, or user-facing copy, always select `.agents/skills/editing-final-state-content/SKILL.md` before editing.
- If a task involves implementation design, dependency selection, or cross-surface changes in `server/`、`web/`、`launcher/`、storage、render, always select `.agents/skills/glue-coding/SKILL.md` before planning or coding.
