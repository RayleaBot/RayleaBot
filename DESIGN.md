...
Web 当前已落地 M1 产品组件基础、M2 协议中心、M3 应用壳及认证入口，以及 M4-A 插件工作区。插件列表、商店、详情、全局插件设置、菜单中心和指令中心已使用产品组件，AppCard、AppStatusTag、RetryPanel、管理上下文操作与插件管理面 Host 同步迁移。对象卡片、分区表单、控制台虚拟列表与独立 iframe 边界保持原有职责。

账号、治理、日志和系统等其余工作区仍待 M4-B/C；顶层 App 的 Ant Design Vue provider 与旧样式入口保留至 M5 清理，独立插件 iframe 内部的组件库不属于 Web 宿主迁移范围。本文记录已落地范围，后续阶段按实际实现更新，迁移范围见 [Web UI 迁移说明](docs/design/web-ui-migration.md)。
...
AppInput 的布局容器样式与内层字段样式分开，前缀图标不接收指针操作，也不替代标签。AppSelect 保留字符串、数字和布尔值的原类型，尚未包含在选项中的已选值继续显示；可清除的多选在清除后将焦点归还选择器。[`AppNumberInput`](web/src/components/AppNumberInput.vue) 按业务需要启用可空值，不将空白输入自动当作零；限流字段将次数、时间窗和单位分别标注，窄屏改为单列。
