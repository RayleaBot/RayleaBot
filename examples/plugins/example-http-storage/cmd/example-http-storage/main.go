...
func handle(ctx context.Context, event *rayleabot.EventContext) error {
	...
	_, _ = event.Actions().LoggerWrite(ctx, rayleabot.LoggerWriteRequest{Level: "info", Message: "The HTTP response was cached at " + path + ".", Fields: map[string]any{"status_code": result["status_code"], "cached_path": path}})
