...
- 每个插件可声明一个内置管理页入口，并可在同一入口目录内声明多个工作区页签。
...
- `pages`：插件详情页工作区页签。省略时使用 `entry` 和 `label` 显示单个页签。
- `pages[].id`：稳定页签标识。
- `pages[].label`：页签标题。
- `pages[].entry`：插件包内相对路径，必须是 HTML 文件，且必须位于 `management_ui.entry` 所在目录。

## 静态资源

- 正式公共路由前缀：`/plugin-ui/{plugin_id}/...`
- 宿主只读取 `management_ui.entry` 所在目录下的资源文件。
- `pages[].entry` 不扩大静态资源读取范围。
- 不提供目录枚举。
