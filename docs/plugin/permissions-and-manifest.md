...
插件进程是管理员信任的本地代码，不是操作系统沙箱；插件自行发起的网络、子进程和临时文件操作不经过宿主 action 校验。安装签名和摘要证明来源及文件完整性，不构成代码安全证明。
...
插件身份、版本和管理面来自 `info.json`，不在 artifact 重复。入口必须是目标平台可执行格式；管理 UI、模板、资源、许可证、notices 和 SBOM 都必须进入精确文件清单。
