...
- 提供 Windows 专属的桌面启动器，承担基础启动与管理入口。
...
- `Desktop Launcher` 为 Windows 专属；Linux 端以 `raylea-server` + `systemd` 服务 + Web 面板为主，不交付桌面启动器。
...
| Launcher | 提供本地进程启停、环境完整性检查（含 `.deps/`、Chromium、渲染模板资源）、新版本提示和打开 Web 入口（Windows 专属，详见 3.13） |
...
- 托管运行时插件：使用平台内置 Python / Node.js 运行时，由平台统一调用，面向普通用户。
- 二进制插件：由 Go / Rust / C# 等语言编译为独立产物，直接运行，面向普通用户。
- 开发者源码插件：依赖系统环境，仅开发模式使用，允许更灵活的依赖安装和调试。

设计要求：

- 面向普通用户的发行包优先支持“托管运行时插件”和“二进制插件”。
- “开发者源码插件”不作为正式发行包的默认承诺能力。
- v0.1 对二进制插件保留发布形态和 manifest 兼容性，不要求与托管运行时插件具备同等成熟的安装、调试和热重载体验。
- v0.1 的核心验收闭环仍聚焦 Python / Node.js 托管运行时插件；二进制插件不进入首版核心验收路径。
...
- 监控插件健康状态和退出码。
- 执行退避重试并上报状态变化。
- 在拉起托管 Node.js 插件子进程时，必须强制附加 `--max-old-space-size=<limit_mb>`（或等价 `NODE_OPTIONS`）启动参数，对单插件 V8 Old Space 做硬上限约束；默认值建议为 `256 MB`，并允许通过 `config/user.yaml` 中的 `runtime.nodejs_max_old_space_size_mb` 受控调整，避免多个 Node.js 插件并行时挤爆宿主机物理内存。
...
- 对托管 Node.js 插件的内存上限，平台配置值是唯一来源；插件 manifest 不得自行声明更高的 V8 堆限制来绕过平台保护阈值。
...
- 平台托管的 Python / Node.js 运行时。
- 平台下载的插件依赖缓存。
- 平台为插件准备的非用户直接编辑资源。

约束：

- v0.1 仅支持平台托管的 Python / Node.js 运行时。
- 不支持自动安装 Rust / Go 工具链。
...
- `.deps/` 中的 Python 运行时负责提供基础解释器和可复用的下载缓存，不作为所有插件共享依赖安装目录。
...
- v0.1 强制仅使用 `npm` 作为 Node.js 插件依赖安装器，避免引入 `pnpm`、`corepack` 和额外环境垫片导致的受控运行时复杂度上升。
...
- 其他 Node.js 包管理器如 `pnpm`、`corepack` 等扩展路径，放到 `v0.2+` 结合受控运行时策略统一评估。
...
- Node.js 插件的 `node_modules/` 不应把“依赖声明兼容”视为宽松复用条件。只要 `package.json` / `package-lock.json` 中的依赖集发生变化，或托管 Node.js 运行时 ABI 发生变化，平台就应在临时目录执行全新的 `npm ci` / `npm install` 后再原子替换，避免 `NODE_MODULE_VERSION mismatch`、原生扩展残留和玄学崩溃。
...
| 进程衍生 | 托管 Python / Node.js 插件不得依赖再启动额外子进程作为受支持能力；Runtime 管理的插件主进程是 v0.1 唯一正式承诺的执行边界 | 预编译二进制插件不属于本规则约束范围，但也不进入 v0.1 主验收路径 | `sandbox.child_process_unenforced` | 必须标记为“衍生进程不可受控”，并在插件详情、环境检查和诊断包中统一暴露 |
...
- Launcher 打开 Web UI 时，优先通过一次性 Token 或本地 Session 引导用户进入管理页面。
...
- Launcher 可直接打开初始化页面，本机浏览器也可通过本地地址访问初始化向导。
...
- Launcher 附带的一次性 Token 只用于本机引导进入登录或初始化流程，不等价于长期管理 Session；完成校验后仍应换发标准管理会话。
- 若 `setup_required` 阶段的 Launcher 一次性 Token 已过期、失效或被重复使用，Web UI 不应白屏、死循环跳转或只返回裸 `401`。应展示本机可读的友好错误页，明确提示“安全验证已过期，请在服务器上重新打开 RayleaLauncher 以继续初始化向导”，并允许用户返回本机初始化入口重新获取新 Token。
...
| 必须重启服务 | Web 监听地址 / 端口、SQLite 路径、关键目录根路径、受控运行时根路径 | 明确要求重启主服务后生效，不允许实现层静默热切换 |
...
| `platform.resource_missing` | 必要运行时资源缺失 | 阻止服务进入 `running`，提示补齐 |
...
- 启动器是 v0.1 的 Windows 专属配套能力，不是单独的高级运维平台。
- Windows 端以启动器作为完整体验入口。
- Linux 端不交付 GUI 启动器，统一以 `raylea-server` 服务进程 + Web 面板为基础体验。
- Linux 端应提供 `systemd` 服务模板或安装脚本，适配 LXC / 家庭服务器自托管场景；`nohup` 仅作为临时调试入口。

#### 3.13.2 职责范围

- 启动 / 停止 / 重启 Bot 服务。
- 打开 Web 管理面板。
- 本地环境完整性检查，包括 `.deps/`、Chromium、Python / Node.js 运行时以及内置渲染模板资源是否齐备。
...
- 查看启动失败摘要或极简本地尾部日志。
- 新版本提示与基础离线版本比对。
- Windows 用户文档应明确说明：Launcher、本地托管运行时和受控 Chromium 首次运行时可能触发 Defender / SmartScreen 扫描或误报提示；用户应通过 `release_manifest.json` 与 `SHA256SUMS.txt` 校验正式发行包，而不是被引导去关闭系统安全防护。
...
- 打开 Web UI：Launcher 向服务端 `POST /api/session/launcher-token` 获取一次性 Token，拼接为 `http://127.0.0.1:{port}/?token={one_time_token}` 后调用系统默认浏览器打开。
...
- 桌面启动器：`.NET 10` LTS 线 + Avalonia `11.3.12`，仅交付 Windows 版本。
...
| 浏览器控制 | `chromedp` `v0.13.2` | CDP 协议封装，用于渲染引擎 |
...
| Launcher | `.NET 10` LTS 线 + Avalonia `11.3.12` | `.NET 10` 官方支持到 `2028-11-14`，比将于 `2026-11-10` 结束支持的 `.NET 8` 更适合作为新项目基线；Avalonia `11.3.12` 对 `net10.0` 兼容 |

补充约束：

- `当前稳定版` 只适用于前期选型讨论；一旦进入 v0.1 正式实现，必须把 Go、Node.js、pnpm、Python、.NET SDK、Avalonia 包版本、Vue 主版本等写入仓库中的明确基线文件或工程文件，不再依赖口头约定。
- 进入实现后，4.1.1 中带有“或”的依赖选项必须收敛为单一选择，并同步体现在 `go.mod`、`package.json`、`global.json`、CI 工作流和文档中；不得让 AI 在不同模块中自由选择不同库。
- 应新增一份独立的工程基线文档，例如 `docs/engineering/baseline.md`，记录工具链版本、包管理器、构建入口、目录职责、本地开发命令，以及 `.deps/manifest.json` 中受控运行时资源的版本与校验信息，作为 AI 实现时的第一优先上下文之一。
- Web、Server、Launcher 各自只允许一种默认构建入口，例如 `go build`、`pnpm build`、`dotnet publish`；不得同时维护多套等价但不一致的脚手架命令。
...
- 托管 Node.js 插件的依赖安装链路固定为“受控 Node.js 运行时 + `npm` 安装器”；原因是 `npm` 随 Node.js 一起分发，更适合作为 Runtime Manager 的最低依赖。官方 Node 插件模板和示例必须兼容该安装链路。
- 托管 Node.js 插件的运行链路固定由 Runtime 注入 `--max-old-space-size=<limit_mb>`；默认以 `runtime.nodejs_max_old_space_size_mb = 256` 作为单插件 V8 堆上限，避免多个 Node.js 子进程在小内存宿主机上叠加触发 OOM。
- 托管 Python 插件的依赖安装链路固定为“受控 Python 运行时 + 每插件独立 `.venv/` + `pip` 安装”；不采用共享站点包目录，避免插件间依赖污染。
- `.deps/` 中受控运行时资源的来源、版本、校验值和适用平台必须写入 `.deps/manifest.json`；Launcher、CLI `doctor` 与发布流程应复用同一份清单，而不是各自维护独立常量。
...
| `launcher/global.json` | .NET SDK 基线 | 固定 `.NET 10` SDK feature band，不依赖开发机默认 SDK |
| `launcher/Directory.Packages.props` 或等价集中包版本文件 | Launcher NuGet 依赖基线 | 固定 Avalonia 与其他 NuGet 包版本，不允许散落在多个项目文件中各自漂移 |
...
- `.NET 10 SDK` 可用，能执行 `dotnet --info` 并识别 Launcher 工程。
- 本地已具备用于渲染调试的 Chromium 或受控浏览器运行时；Web E2E 场景还应准备 Playwright 浏览器二进制。
- 开发机应能执行基线命令：`go test ./...`、`pnpm install --frozen-lockfile`、`pnpm build`、`dotnet test ./launcher`；若其中任一命令失败，不应开始实现业务代码。
...
- `.deps/` 由发行包或启动器准备受控运行时和依赖缓存，不作为通用工具链安装目录。
- `.deps/manifest.json` 应记录受控运行时资源的版本、下载来源、SHA256 和适用平台，至少覆盖 Chromium 运行时以及后续可能托管的语言运行时补充资源。
- `Plugin Runtime` 作为主服务内的 Runtime Manager 依赖这些受控资源，不负责自举完整环境安装逻辑。
- 渲染引擎应作为受控组件随发行包或首次启动准备，不应要求插件自行携带浏览器内核，也不默认依赖用户系统已安装浏览器。
- `.deps/` 需要托管受控 Chromium 运行时及相关缓存；浏览器运行时会显著增加发行包体积，具体大小随平台和构建方式变化。
...
- v0.1 内嵌的受控 Chromium 运行时只承诺覆盖 `x86_64 / amd64` 架构；ARM64 环境默认不保证随包提供可用 Chromium。
...
- 推荐配置：`2 vCPU / 2 GB RAM / 5 GB+` 可用磁盘，适合常驻 Chromium、多个托管运行时插件和较稳定的 Web 管理体验。
...
| `.deps/` | 否 | 否 | 平台托管 Python / Node.js / Chromium 运行时及依赖缓存目录 |
...
| --- | --- | --- | --- |
| `windows-x64-full` | `zip`，含 `raylea-server.exe`、`RayleaLauncher.exe`、Web UI、官方内置插件、`.deps/` | First-class | `windows_full_smoke` |
| `linux-x64-server` | `tar.gz`，含 `raylea-server`、Web UI、官方内置插件、`systemd` 示例文件、受控运行时资源 | First-class | `linux_server_smoke` |

说明：

- ARM64 在 v0.1 不进入 First-class 发行矩阵；如后续提供试验性产物，必须在 manifest 中明确标注 `support_level: experimental`，且不得与正式产物混淆。
...
- `.deps/`：如希望避免重复下载托管运行时和依赖缓存，可作为可选持久化目录挂载。
...
| 跨平台恢复 | 仅配置与业务数据参考恢复 | `.deps/`、托管运行时与二进制插件不保证可直接复用 |

- 恢复完成后，旧的 `admin_sessions` 与一次性 Token 必须全部失效，避免把旧会话直接带回新实例。
- `.deps/`、Chromium、Python / Node.js 托管运行时默认允许在恢复后按需重建，不要求随备份包一起恢复。
...
3. 再执行运行时、模板资源、受控 Chromium 与插件兼容检查；不兼容的插件应保留包与数据，但默认保持 `disabled` 或 `upgrade_pending_regrant`，不得自动启用。
...
- 对 ARM64 Linux / LXC 场景，如发行包未提供受控 Chromium，部署文档应明确要求用户先安装宿主机可用的 Chromium，并通过 `render.browser_path` 指向实际可执行文件路径。
...
- `ci` 应显式安装并校验固定技术基线，至少覆盖 Go `1.25.8`、Node.js `24.14.0`、`pnpm 10.32.1`、Python `3.12.13` 与 `.NET 10` SDK；一旦仓库中的 `go.mod`、`packageManager`、`global.json`、Python 运行时配置与工作流声明不一致，CI 应直接失败。
...
| --- | --- | --- | --- | --- |
| `lint` | Go / Web / .NET 格式校验、静态检查、基础类型检查 | `ubuntu-x64` | 是 | 最快失败层，优先暴露低成本问题 |
...
| `ci-launcher` | `dotnet test ./launcher`、`dotnet publish ./launcher -c Release` | `windows-x64` | 是 | Launcher 仅在 Windows 上正式门禁 |
| `smoke-pr` | 轻量闭环 smoke：初始化、登录、插件列表、任务流、`/healthz` / `/readyz`、最小渲染验证 | `ubuntu-x64` | 是 | PR 级别只跑最小闭环，不跑最重恢复场景 |
| `nightly` | 重型 E2E：备份 / 恢复 / 迁移、插件安装、重载、重渲染 golden、Windows 启动器联调、Playwright 页面回归 | `ubuntu-x64`、`windows-x64` | 否 | 夜间回归层，用于覆盖高成本场景 |
| `release` | 构建正式产物、校验 `release_manifest.json` / checksum、运行交付矩阵 smoke profile 后发布 | `windows-x64`、`linux-x64` | Tag 门禁 | 不通过则不得发布正式 Release |
...
- v0.1 不要求 ARM64 进入 PR 门禁；如后续提供 ARM64 构建，可先纳入 nightly 的非阻塞 build-only 检查。
...
| Launcher 一次性 Token 过期 | Web UI 显示友好提示页，不陷入空白或裸 `401` 死锁 |
...
9. 发行包内置或按需准备的受控 Chromium 运行时可独立完成图片渲染，不依赖系统已安装浏览器。
