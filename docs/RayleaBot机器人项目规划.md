...
| `4.12 风险与缓解措施`、`4.13 v0.1 验收标准` | [`release/acceptance-and-risks.md`](./release/acceptance-and-risks.md) |

## 三、核心子系统专题入口

### 3.1 协议适配层

RayleaBot v0.1 以 OneBot11 反向 WebSocket 作为唯一正式接入路径，事件归一化、入站 / 出站消息模型和管理 WebSocket 事件统一收口到架构专题。
...
v0.1 的主要运行风险和缓解方向收口在 release 验收与风险文档。
文档：[`release/acceptance-and-risks.md`](./release/acceptance-and-risks.md)

### 4.13 v0.1 验收标准与质量保障场景

v0.1 的发布门槛、关键场景和验收结论收口在 release 验收与风险文档。
文档：[`release/acceptance-and-risks.md`](./release/acceptance-and-risks.md)
...
- 补齐治理闭环，`/governance` 承担治理管理，`/commands` 保留命令治理读面
...
- RayleaBot 首阶段优先完成稳定、可管理、可扩展的自托管机器人基础平台。
- 插件系统是长期核心能力，但首版范围继续收敛在 Python / Node.js 的完整闭环。
