...
- 额外语言插件运行时、源码插件与安装脚本；
- OneBot11 与 QQ 官方机器人之外的聊天协议；
- 跨适配器的身份合并：不同适配器下的用户与会话标识各自独立，不承诺映射为同一主体；
- 远程组件运行时或第二套状态模型。

第三方插件是用户检查来源、artifact 摘要和能力后确认执行的完全可信本地代码。独立插件域只隔离管理页的浏览器权限，不承诺把后端插件变成不可信沙盒代码。
...
各目录的文档索引见仓库 [README](../README.md)；已发布版本记录见 [`CHANGELOGS/`](./CHANGELOGS/README.md)。
