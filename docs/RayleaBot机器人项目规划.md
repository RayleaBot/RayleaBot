...
| [`plugin/`](./plugin/README.md) | 插件生命周期、manifest、能力声明、能力参数、协议与 SDK |
...
插件运行时、后台任务和 OneBot11 连接状态使用统一状态模型，并投影到管理面。
...
插件 manifest、能力声明、能力参数和命令声明继续以正式 schema 维护。
...
平台优先稳定统一事件模型、manifest、插件协议、能力声明、能力参数和渲染接口，其余扩展按正式边界后置。
