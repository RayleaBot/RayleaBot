...
本页说明 RayleaBot v0.1 的正式验收口径、关键场景和主要风险。
...
v0.1 的发布门槛不是“开发者环境能运行”，而是用户能按受支持文档完成安装、初始化、插件启用、基础管理和恢复排障闭环。

## 核心验收场景

- OneBot11 反向 WebSocket 能建立受控链路。
...
- Launcher 检测到现有服务、启动失败或自动登录失败时，都会给出可读提示。
...
- 多协议、多实例、更宽动作族和平台级 LLM 能力不属于 v0.1 验收范围。
