...
| `/governance` 页面职责 | 当前管理面展示治理摘要，并承担黑白名单管理 |
...
| Web 工作区 | `/governance` 展示治理摘要并承担黑白名单管理，`/commands` 展示有效命令策略与全部声明命令 |
| 默认权限与冷却 | 已通过现有配置模型管理 |
...
| `/governance` 工作区补齐 | ☑️ | 读取、编辑、提交结果、错误提示和空态保持在同一治理工作区内 |
...
| 场景 | 验收目标 |
| --- | --- |
| 治理工作区 | `/governance` 能完成治理读取、黑名单管理、白名单管理和错误反馈闭环 |
