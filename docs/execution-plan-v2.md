...
- 状态：本轮补充评审的残留与新增问题已完成核实和收敛，逐项结果见[补充核实台账](execution-plan-v2-reconciliation.md)。P26 当前源码的分发验收仍须在发行前执行；原候选证据见[验收记录](release/validation-v0.4.0.md)。
- 执行与完成记录：本文件。三份输入评审保留为历史依据，不再分别回写其阶段清单。
- 原计划代码基线：`809fec3a`；补充评审基线：`f782207d`；评审归档提交：`2c358c93b55c0ad014b94d4f597ed797c8144a17`。
...
| 属性测试与等待 V1 | catalog 的启用、禁用及不存在用例使用 rapid 生成器，但旧 ConcurrentSafety 无生成器也无业务断言；lifecycle 的 5.25s 等待覆盖 5s 退出预算回归 | 保留真实属性测试，并发用例已在补充收敛中加入状态/成员/快照断言和 race；等待按事件与预算处理，不按时长清零 |
| 前端组件 Q7 | `ui/*` 为基础组件，AppButton/AppDialog 有加载、无障碍、焦点和退出行为 | 保留基础层与产品封装，按真实职责删冗余；不执行“组件层二选一” |
| E2E T5 | production 原本混合 real-server 与 plugin-ui-fixtures；现只保留真实 Server，受控展示场景统一为 ui-fixtures | `d09e5a75`：10 个真实 Server、110 个受控浏览器用例通过；移除 mock 的凭据、治理、秘密合并与安装规则副本，详见 C08 |
...
| [x] | P05 | P1/L | 错误码与结构化失败语义 | P01 | 完成 | 补充收敛 `823d80a4` / `ef78b623`：恢复诊断、SDK shutdown、HTTP 码适用范围及实际产生码的架构检查闭环；缺少更新服务在装配时失败，凭据规则归 auth。详见 C01/C05。 |
| [x] | P06 | P1/L | 契约、样例与校验覆盖 | P01 | 完成 | 补充收敛 `272af5ca`：适配器鉴权和 initialization_failed 诊断补齐，WS/HTTP/manifest 按共享静态声明和运行投影引用，严格契约与保护路由对照通过。详见 C02。 |
| [x] | P07 | P1/L | 错误目录、协议与版本生成链 | P05、P06 | 完成 | 补充收敛 `c2a52049` / `99004a78`：协议动作分类及发布 artifact 标识来自契约生成。详见 C03/C10。 |
| [x] | P08 | P1/L | 默认配置、类型化与热更新 | P05、P06 | 完成 | 补充收敛 `ef78b623`：HTTP 动作超时/重试默认值读取 config schema；管理层无消费者热更新导出删除。详见 C05/C12。 |
| [x] | P09 | P1/M | 数据初始化基线与历史代码清理 | P06、P08 | 完成 | 67113c74；目标schema初始化与旧密码/迁移删除；空目录、当前格式恢复及默认/相对/绝对DB重启验证通过 |
| [x] | P10 | P1/L | 文件路径与归档原语 | P04、P06 | 完成 | 补充收敛 `48659dd4`：渲染、管理静态资源、安装和更新的包含判断/摘要统一，保留边界策略；目录摘要、大小和取消用例通过。详见 C04。 |
...
| [x] | P13 | P1/L | 插件状态、生命周期与装配 | P02、P04、P12 | 完成 | 补充收敛 `548f53cd` / `8d771bbe`：安装阶段及回滚材料归属明确，生命周期完整 race 和真实安装集成通过，取消不会误报摘要不一致。详见 C12。 |
...
| [x] | P16 | P2/M | 模型、视图与辅助依赖收回 | P12、P13 | 完成 | 补充收敛 `c2a52049`：权限/日志/凭据 SQLite 实现独立，菜单与配置运行时不带入 render，Ingress 使用实际接口；五个目标包的 go list -deps 均无 storage/sqlcgen/render。详见 C03。 |
...
| [x] | P18 | P2/M | 运维、调度与 CLI 业务归属 | P09、P16 | 完成 | 补充收敛 `aa6bb940`：恢复主流程及归档检查复杂度 115→19、64→7，写入/取消/回滚失败、CRC、锁和 Windows junction 用例通过。详见 C12。 |
| [x] | P19 | P2/L | 后端目录分组迁移 | P08、P10–P18 | 完成 | 按 §2.1 完成 30 项目录映射及四个包名调整；475 份生产 Go 的 AST 主体不变；生成/SQL/结构正反例/strict/完整 Server 测试与三平台 Server、Updater 构建通过 |
| [x] | P20 | P1/L | 测试体系替代与离线回归 | P01；随各行为包推进 | 完成 | 补充收敛 `a68cda1d`：删除两个 testCatalog 副本，补齐并发业务断言，清理用普通文案识别错误/事件的断言；QQ live 显式登记为人工 smoke。详见 C11。 |
| [x] | P21 | P2/M | 死代码、重复 helper 与集成精简 | 相关行为包、P20 | 完成 | 补充收敛 `48659dd4` / `ef78b623` / `548f53cd`：下载转发、黑名单错误例外、管理测试专用导出和重复指针助手删除；两处确认空目录删除。详见 C04/C05/C12。 |
| [x] | P22 | P1/L | Web 状态、错误、请求与真实 E2E | P05–P08、P12 | 完成 | 补充收敛 `3c834add` / `d09e5a75`：完整日志工作区与名单模板共享，HTTP 模板/WS 契约生成、locale/时间/色调/断点统一；466 单测、构建、10 真实 Server 与 110 受控浏览器用例通过，详见 C06–C08。 |
| [x] | P23 | P1/L | Launcher 类型边界与失败可见性 | P05、P06、P11 | 完成 | 补充收敛 `15e1b33b` / `99004a78`：本机模型/8组枚举由 Go 绑定派生，前端65项及Go测试通过；Windows助手解析与Linux/macOS引导分流明确。原候选的原生界面证据不代表本轮重新打包。详见 C09。 |
| [x] | P24 | P2/M | 工具链、开发与发布脚本收敛 | P07–P11 | 完成 | 补充收敛 `5ca2cccd` / `6e3e0d02` / `99004a78` / `e133093e` / `e7e15d48`：工具链和 artifact 标识同源，归档内容策略合并，签名故障中止构建，发布者证书约束区分主程序与依赖。详见 C10。 |
| [x] | P25 | P2/M | 文档、文案、资产与说明归属 | 已完成任务随轮更新 | 完成 | 本次文档归档；当前入口与契约一致、历史设计记录分离、孤儿预览删除、来源/字体核查；143文档链接/指令/设计生成与1023中文字形离线验证通过 |
| [ ] | P26 | P1/L | 全新分发验收与交付准备 | P01–P25 | 当前源码待验收 | 历史候选 0.4.0 / 5b2fa9d8 的四包、恢复/运行、元数据和 Windows 界面证据见[交付记录](release/validation-v0.4.0.md)。本轮补充收敛未将该产物认定为当前源码产物；发行前需绑定新提交重新执行分发验收。 |
...
- 无生成器的 rapid 包装逐例识别；保留 catalog 中实际生成输入的用例，并重写缺少业务断言的并发用例。固定 sleep 用事件/预算控制替代，保留原回归的时序条件与最终超时上限。
...
| 1.5 property、sleep、文案断言、health fixture 自证 | P20；区分实际使用生成器的 catalog 用例与缺断言的旧并发用例；领域 readiness 增真实依赖降级覆盖，编码 fixture 继续验证 wire shape，契约承载的文案断言可保留 |
...
| 2026-09-10 | P08 配置 / P09 初始化 | 待验收 | 工作区，待分组提交 | schema默认+user.yaml；删除旧迁移与密码分支；schema_metadata单一基线；配置保存与运行快照分离；热更新失败返回failed_groups | SQLite/auth/config/configruntime/backup/recovery/CLI相关测试与race、sqlc无漂移；真实Server+当前SDK插件备份恢复并重启两次，演练覆盖配置、插件数据与恢复后登录 | 样本使用隔离临时数据；四平台正式签名产物尚未验收 |
...
| 2026-09-10 | P18 离线恢复 / 管理员重置 / 调度视图 | 待验收 | 工作区，待分组提交 | recovery负责隔离预检、os.Root边界及文件事务；auth负责锁与凭据事务；scheduler负责展示与时区；CLI/HTTP保持边界 | 实际Server自定义相对DB和绝对DB便携重定位恢复通过，覆盖默认及自定义数据库落点；源DB摘要不变；Windows实际junction越界回归通过 | 标准symlink测试需Windows权限已明确跳过，junction已真实执行；最终组合race在协议生成完成后执行 |
...
| 2026-09-10 | P09/P10/P18/P24 收口 | 完成（本机部分） | `67113c74`、`4be30685`、`78ea5106`、`f2ae35d3`、`2a6cd692`、`57d6bf78` | 当前格式初始化恢复、受限归档、领域运维服务、工具链入口和统一分发矩阵 | 空目录与自定义DB实际恢复；Windows事务回滚/race；Python75项、开发工具及Git Bash显式路径正反例 | 原生Linux/macOS及四种0.4.0产物仍由P26完成；不把交叉编译或测试版本包记为正式分发验收 |
...
| 2026-09-10 | P25 当前文档与资源来源 | 完成 | 本次文档提交 | JSONL v3/多适配器/语言中立artifact/HTTPS摘要/停服CLI与当前初始化说明；历史design-qa归档、未引用排行榜预览删除；0.4.0候选说明与索引 | 143份文档链接、agent-docs、设计token/Launcher图标、排行榜真实渲染；101份字体来源授权及1023中文离线字形检查通过，无网络或系统字体回退 | 当前源字形样本不代表全部Unicode；候选说明不宣称公开分发或最终平台验收完成 |
| 2026-09-10 | P23/P26 Windows 实包补验 | 待验收 | UI包源码 `db938746`；功能包源码 `9f49b885`、harness `07a365d1` | 实际Wails窗口/托盘/第二实例/启停/退出；原生插件安装设置重载卸载、两次PNG及所属资源回收 | UI证据归档及包摘要见[验收记录](release/validation-v0.4.0.md)；功能包两张960×411 PNG、原生插件及浏览器回收、本版备份/重启通过 | 本机功能观察仅1秒且使用已校验资源缓存；不替代四种最终产物的300/600秒门槛 |
| 2026-09-10 | P01/P24/P26 托管验收反馈 | 待验收 | `8e43b34f`、`44ff2dc1`、`80aaa1c7` | 修复Wails CLI GTK3标签、macOS Bash参数及Windows文件URL盘符 | 两轮CI的生成/脚本/客户端与平台结果、Windows archive/recovery部分通过及self-host失败均按实际源码登记于[验收记录](release/validation-v0.4.0.md) | 最终Windows Chromium启动及四种完整产物仍需通过结果；不据局部通过勾选整包 |
| 2026-09-10 | P13/P26 插件末帧与退出顺序 | 完成（修复子项） | `30734d80` | 显式拥有stdout/stderr，退出/Stop先有界等待协议分类；保留合法最后结果与正式protocol_violation | 旧实现确定性回归失败；修后100轮末帧、10轮关键race，runtime/lifecycle/dispatch/App/integration/ws race及runtime lint通过 | Linux最终结果随下一轮CI登记；不代表P26完整验收完成 |
| 2026-09-10 | P24/P26 解包与真实定时回调 | 完成（验收子项） | `b4a23bde`、`c1f1993f` | Windows解包短路径移动复用有界目录锁重试；原生插件通过SDK创建分钟cron并接收实际scheduler.trigger | 从新ZIP解包的Windows功能流程通过：任务/插件PID与scheduler.internal来源一致、success=1、卸载后任务消失；105项发布脚本测试（1平台skip）、strict和独立fixture构建通过 | 本机主观察窗口1秒、资源用已校验缓存；新增调度用例仍须进入最终同源四包验收 |
| 2026-09-10 | P17/P26 浏览器资源等待 | 完成（修复子项） | `733f8fa6`、`65b92247`、`898a28b7` | CI原生浏览器测试独立成步；启动遵循调用方预算；两处Promise显式等待与每target活跃仿真 | 旧实现延迟字体/图片门控连续三轮提前返回PNG；修后42次真实浏览器race通过，含双tab各字体/图片独立放行及关闭回收，lint0 | 未增加30秒渲染上限或串行化worker；托管Page.captureScreenshot合成超时仍另行处理 |
| 2026-09-10 | P26 第三轮原生产物 | 待验收 | 源码 `30734d80`；run `34535383597` | Linux full/server、macOS full三份中间候选完成原生构建与完整流程，归档摘要及metadata独立核对 | 三平台archive smoke、300秒恢复观察及600秒self-host通过，详见[候选验收记录](release/validation-v0.4.0.md)；Mac为flags=0x20002的链接器ad hoc签名，页哈希通过 | Windows在30秒内截图未完成，四包assemble跳过；聚合签名未生成，Mac无DeveloperID/CMS/TeamID且公证/Gatekeeper未验收；不作为最终分发 |

| 2026-09-10 | P06/P22 最终定向补验 | 完成 | `d5493776`；生产实现 `5b2fa9d8` | 调度成功样例与实际task_id/next_run一致；补齐窄屏、减少动画、高对比度与键盘现有用例 | 生成verify、strict、Server/独立SDK向量通过；5个现有浏览器用例21.9秒全过，端口/子进程释放 | 仅fixture/testdata变化，不改变候选生产编译输入 |
| 2026-09-10 | P01/P23/P24/P26 最终候选验收 | 完成 | 生产 `5b2fa9d8`；发布工具 `45508122` | 完整CI与四种原生产物；Windows原生窗口/托盘/退出；签名清单按各包原始字节生成并核对 | CI `34539781821`、四包 `34539781886`、受控重签 `34543000042` 全通过；逐包SHA/size/build_info/内层摘要、交付Server验签、实际cron与进程回收通过，详见交付记录 | Windows保持guided，Mac仅ad-hoc；最终Windows第二实例重复复验被自动审批拒绝，未执行，保留此前未变实现证据；外部实网与Linux/Mac原生UI不冒充通过 |
| 2026-09-10 | 补充评审 C01–C12 | 完成 | 后端及工程 `823d80a4` 至 `197f2234`；Web `3c834add` / `d09e5a75` | 错误与契约、传递依赖、文件原语、领域规则、复杂度、工具链、Web 共享工作区及真实 Server 测试边界闭环 | Server/SDK 全量、相关 race、核心 lint、466 Web 单测、10 真实 Server 与 110 受控浏览器用例及生成/契约门禁通过；详见[补充核实台账](execution-plan-v2-reconciliation.md) | 实网人工 smoke、Docker 镜像及当前源码正式分发包未执行；旧 P26 包不代表本轮新源码 |
...
当前无需新的实施取舍。P26 本轮目标版本确定为 `0.4.0`；维护者已授权代理自行选择验收环境，实际可用签名/原生环境在验收记录中核实；独立插件构建接入按本仓库边界执行；性能试验等技术选择按各任务证据决定，只有出现新的产品语义或资源约束才再次询问维护者。
