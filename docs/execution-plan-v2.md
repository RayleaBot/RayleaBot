...
- 状态：P01–P26 已完成；本轮交付 0.4.0 未公开分发候选包，验证边界见[验收记录](release/validation-v0.4.0.md)。
...
| [x] | P23 | P1/L | Launcher 类型边界与失败可见性 | P05、P06、P11 | 完成 | fa97b713 / 5b2fa9d8；API/绑定、坏文件/关闭、JS/Go/race/Renderer与三平台原生构建通过；最终Windows实包启停/托盘/退出通过；单实例有此前未变实现的原生证据，本次重复复验被自动审批拒绝，见验收范围 |
| [x] | P24 | P2/M | 工具链、开发与发布脚本收敛 | P07–P11 | 完成 | 2a6cd692 / 57d6bf78 / 45508122；双平台脚本、三平台原生构建、四包恢复/运行通过；修正CRLF资源清单摘要并对原归档重新生成元数据、签名与独立验签 |
| [x] | P25 | P2/M | 文档、文案、资产与说明归属 | 已完成任务随轮更新 | 完成 | 本次文档归档；当前入口与契约一致、历史设计记录分离、孤儿预览删除、来源/字体核查；143文档链接/指令/设计生成与1023中文字形离线验证通过 |
| [x] | P26 | P1/L | 全新分发验收与交付准备 | P01–P25 | 完成 | 0.4.0 / 5b2fa9d8四包：完整CI、300秒×2恢复与600秒运行、真实插件/PNG/定时回调/资源回收通过；45508122修正元数据，受控重签和逐包独立核验通过；产物、Windows UI及分发限制见[交付记录](release/validation-v0.4.0.md) |
