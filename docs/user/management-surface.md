...
| 权限策略 | `/permission-policy` | 查看和保存超级管理员、默认权限、用户命令速率、群命令速率和冷却提示 |
...
- `/permission-policy` 显示策略总览，包含超级管理员数量、默认权限、用户冷却、群冷却和冷却提示。
- `/permission-policy` 提供超级管理员、默认权限级别、用户命令速率限制、群命令速率限制和冷却提示配置。
- 保存后页面显示配置保存结果，并刷新当前生效命令策略摘要。
...
- 权限策略承担超级管理员、默认权限和命令冷却配置；黑白名单承担名单条目和白名单启用状态。
