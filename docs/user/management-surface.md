...
- 插件页提供安装、卸载、启用、禁用、重载、授权、受控 console 与 rich manifest metadata 查看。
...
- 插件详情页显示版本、运行时、作者、许可协议、仓库、主页、关键词、截图路径、系统依赖、`default_config`、`concurrency`、`declared_capabilities`、`dependencies` 和 `scopes`。
- 插件页直接显示每个插件声明的注册命令，并提示同名命令冲突与插件来源标识。
- 指令中心集中展示全部已声明命令，并显示默认权限、冷却配置、当前生效命令策略与 user / group 黑名单。
...
- Launcher 展示会组合服务端正式状态与本地桌面壳观察，但正式服务状态仍以服务端接口为准。
