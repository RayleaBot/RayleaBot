...
- 恢复摘要会标明本次操作属于 `restore`、`upgrade` 或 `rollback`，并列出需要人工处理的资源问题、被跳过的插件、确认状态、最近确认记录和下一步建议。
- 启动后的兼容检查会覆盖运行时资源、模板资源、Chromium 浏览环境与已恢复插件。兼容性不满足的插件会保留安装目录和数据，但默认保持禁用，等待人工处理后再手动启用。
- `post_startup` 恢复摘要在 `degraded` 状态下会稳定保留 `manual_actions`、`next_steps`、跳过插件列表与待确认项；在 `compatible` 状态下不保留人工处理建议。
- diagnostics 导出中的 `recovery-summary.json`、Web 管理面与 Launcher 状态页会投影同一份跳过插件确认状态、最近确认记录和下一步列表。
- Web 管理面提供“重新检查恢复状态”“确认已审阅项”和“准备运行环境”入口；完成运行时准备、插件替换、插件卸载或其他人工处理后，可直接触发 `recovery.recheck`、`recovery.confirm` 或 `runtime.bootstrap` 任务。Launcher 状态页继续提供“重新检查”和“准备运行环境”入口，并可直接打开管理面处理跳过插件。
...
- 长期巡检与恢复 drill 都会在启动前按 `.deps/manifest.json` 准备 Chromium、Python、Node.js 与 npm 环境，并在巡检过程中校验 `runtime.bootstrap`、`recovery.recheck` 与 `recovery.confirm` 的正式任务路径。
