...
- 正式包携带 `.deps/manifest.json`。服务启动后会自动准备 Python、Node.js 与 npm 环境；Chromium 浏览环境、Python 与 Node.js 资源按需下载到 `cache/downloads/runtime/`，并展开到 `.deps/store/<resource-id>/<version>/`。
