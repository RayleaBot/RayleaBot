...
迁移前，服务在数据库同目录保存 `<数据库文件名>.pre-migration-000001-<唯一标识>.db`，并在启动日志记录位置。副本创建或校验失败时不开始迁移；迁移步骤失败会回滚数据库，已校验的副本继续保留。确认新版正常后可手动删除该副本，服务不会自动清理它。退回旧核心时使用旧安装目录或旧格式备份；旧核心不能直接读取 `000002` 数据库。
