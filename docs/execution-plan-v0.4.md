...
16. 保留 `RayleaBot/plugin-catalog` 作为默认官方商店目录和签名发布仓库；主仓库不内置业务插件，插件源码与三平台产物仍由各独立仓库发布。
17. 官方商店目录使用独立于核心更新元数据的 Ed25519 公钥注册表。非官方插件可由官方目录收录为社区插件，也可通过本地或远端 artifact 手动安装；本轮不增加可替换官方信任根的多目录管理能力。
...
| P3 | 服务端插件发现、安装与恢复 | ☑️ 已完成 | manifest 先经 schema 校验再进入类型化解析；所有安装来源统一校验 `min_core_version`；artifact v2、静态模板发现和静态 webhook 注册已接入；默认官方远程 catalog 保留，当前 bootstrap catalog 为空；backup v3 拒绝 v2，同时允许保留旧包事实和持久数据。 | 插件 catalog/artifact/lifecycle/webhook、pluginmarket、recovery、CLI 测试通过；新增最低 Core 版本拒绝测试。 |
...
| P5 | 管理桥接、Web 与 Launcher | ☑️ 已完成 | bridge v3、单入口多页面、permissions、稳定命令 ID、统一触发器、真实命令分组和静态 webhook 信息已完整接入管理 API、Web 与 Launcher。OpenAPI、WebSocket 和 bridge 类型均已再生成；Web 清理了增量缓存掩盖的类型错误，Launcher 原生图标资产按现有母版重新生成。 | Web 非增量 typecheck、61 文件 308 项全量测试、受影响界面 77 项回归和 production build 通过；Launcher typecheck、16 文件 64 项测试、Go 测试和完整 package build 通过。 |
...
| P8 | 官方商店目录与信任边界校正 | ☑️ 已完成 | 保留 `RayleaBot/plugin-catalog` 默认官方目录和远端插件下载流程；release workflow 分别注入核心更新与官方目录公钥，缺少任一注册表时失败；商店、合同说明、工程基线和发布文档已同步为最终语义。 | pluginmarket/lifecycle 测试、合同 self-test/strict、runtime schema verify、release YAML 解析、双公钥实际 Server 构建、文档链接和定向 `git diff --check` 通过。 |
| P9 | v0.4 评审修复 | ⬜ 待处理 | 7 个 major 与同轮 minor 已整理到 [`execution-plan-v0.4-review-fixes.md`](./execution-plan-v0.4-review-fixes.md)。 | 修复计划 F1 至 F8 待执行。 |
...
- artifact v2 只记录 artifact_version、target_platform、entry 和文件清单；插件身份与版本来自 `info.json`。
...
- artifact 保持精确文件清单、大小与 SHA-256 校验，并检查入口为目标平台可执行格式。
...
- catalog v2 的 manifest hash 位于 release，asset 只描述平台、URL、归档大小和归档 hash。
- 默认官方远程 catalog 保留，内置 bootstrap catalog 当前为空。
...
- Server 默认从 `RayleaBot/plugin-catalog` 获取官方静态目录和签名；Web 继续通过 `/api/plugin-store/**` 使用商店，不直连目录或插件资产。
- 官方目录与核心更新元数据分别使用独立的 Ed25519 公钥注册表，发布构建缺少任一注册表时失败。
...
| raylea-plugin | inspect、pack、build-go 及错误路径 | ☑️ 通过 | pluginbuild 单测覆盖项目 manifest 检查、Go 构建、原生 pack、artifact inspect、精确清单、目标格式和错误路径；`inspect --plugin` 实际命令回归通过。 |
| Artifact | Windows、Linux、macOS 目标产物结构与 hash 校验 | ☑️ 通过 | 同一 Go fixture 完成 Windows x64、Linux x64、macOS arm64 交叉构建；逐目标检查原生格式、清单、SHA-256 和 ZIP 入口 `0755`。 |
...
| Web | `pnpm run typecheck`、`pnpm test`、`pnpm build` | ☑️ 通过 | 非增量 `vue-tsc` 通过；61 个测试文件 308 项测试、受影响界面 77 项回归和 production build 通过。 |
...
| 官方插件目录 | 目录信任分离、release workflow 解析与双公钥 Server 构建 | ☑️ 通过 | `RAYLEA_RELEASE_TRUSTED_KEYS` 与 `RAYLEA_PLUGIN_CATALOG_TRUSTED_KEYS` 分别注入对应 Go 包；YAML 可解析，构建产物同时包含两个独立测试 key ID。 |
| 文档与仓库 | agent docs、链接、doctor、`git diff --check` | ☑️ 通过 | agent docs、131 个 Markdown 文件链接、Server structure、固定 Node/Go/Python/pnpm 工具链 doctor、废弃引用、敏感信息定向扫描和 `git diff --check` 均通过。 |

## 验收区

- 最终实现状态：评审修复待完成。
- 未完成或阻塞项：[`execution-plan-v0.4-review-fixes.md`](./execution-plan-v0.4-review-fixes.md) 中的 F1 至 F8 待执行；当前无阻塞项。
- 外部插件同步：不在本次范围内。
- 维护者验收：待柒柒验收。
