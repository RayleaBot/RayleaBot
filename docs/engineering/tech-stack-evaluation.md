...
| Web UI | Vue 3.5.30, Vite 8.0.10, Ant Design Vue 4.2.6, Ant Design Icons Vue 7.0.1, Vue Vben Admin 5.7.0 对齐方案, Pinia 3.0.4, Vue Router 5.0.4, vue-i18n 11.3.0, `@vueuse/motion` 3.0.3, RayleaBot HTTP 语义封装, 原生 WebSocket 受控连接封装, Tailwind CSS 4.x, Vitest 4.1.5, Playwright 1.59.1 |
...
`openapi-typescript` 7.8.0 从 `contracts/web-api.openapi.yaml` 生成 `web/src/types/generated.ts`，覆盖当前 54 个操作的请求 / 响应定义。7 个领域类型文件（common, tasks, plugins, system, logs, config, events）从 `generated.ts` re-export。
...
sqlc v1.29.0 以 `server/internal/sqlcqueries/` 下 7 个 `.sql` 文件（35 named queries）为单一来源，生成 `server/internal/sqlcgen/`。全部 7 个 SQLiteRepository（auth, tasks, scheduler, logging, plugins, pluginkv, pluginconfig）的静态查询均由 sqlc 生成；动态 SQL（`ESCAPE` 子句、运行时 `IN` 列表、动态 `WHERE`）保留为手写。
...
详细工程规则见 [`web-antdv-vben-migration-plan.md`](./web-antdv-vben-migration-plan.md)。
