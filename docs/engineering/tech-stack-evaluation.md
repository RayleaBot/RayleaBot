...
1. 维护 `Ant Design Vue`、`@ant-design/icons-vue`、`@vueuse/motion` 与 Tailwind CSS 4 的固定版本。
2. 在 `web/` 内整理 `layout / adapter / request / stores / access / preferences / views` 分层。
3. 用 Vben 风格应用壳替换当前 `AppShell`、页面壳和全局 provider 组织。
4. 保持 `web/src/lib/http.ts` 与 `web/src/request/http.ts` 的 RayleaBot 请求语义，不改变后端 contract shape。
...
- `web/package.json` 与 `web/pnpm-lock.yaml` 保持当前依赖固定版本。
