...
| 等级 | 改进项 | 影响 | 状态 |
|------|--------|------|------|
| P0 | 移除 `pluginhttp/client.go` 生产路径 `panic` | 避免进程崩溃 | ✅ `parsePrefixes` 返回 error，`init()` 中 `log.Fatalf` |
| P0 | 为 `http.Server` 设置超时和 `MaxHeaderBytes` | 防御资源耗尽攻击 | ✅ ReadTimeout 30s / ReadHeaderTimeout 10s / WriteTimeout 60s / IdleTimeout 120s / MaxHeaderBytes 1 MiB |
| P0 | 修复 `Register` 锁内阻塞 | 消除死锁风险 | ✅ 锁释放后再 close+wait 旧 worker |
| P1 | App 构造期引入资源清理栈 | 防止构造失败时资源泄漏 | ✅ `cleanups []func()` + `abort()` 逆序执行 |
| P1 | 前端 HTTP 封装增加超时与取消 | 防止请求无限挂起 | ✅ AbortController + DEFAULT_TIMEOUT_MS 30s |
| P1 | Web 入口添加 CSP | 降低 XSS 风险 | ✅ `<meta>` CSP：script/style/connect/img/font |
| P1 | 提升 request ID 到 128 位 | 增强日志关联能力 | ✅ 16 字节，移除不可达 fallback |
| P2 | 统一 Server 侧 `message_key` 填写 | 支持前端 i18n 展示 | ✅ 已确认全部 31 处 writeError 均填写 message_key |
| P2 | SQLite 读连接声明只读 | 防御误写 | ✅ `PRAGMA query_only = ON` |
| P2 | 拆分 `api.ts` / i18n 文案 | 降低维护冲突 | ✅ `api.ts` 拆分为 7 个领域文件 + barrel re-export；`zh-CN.ts` 拆分为 9 个领域文件 + barrel 重组 |
| P2 | 规划文档体量 | 降低维护冲突 | — 规划文档拆分不在本轮范围 |
