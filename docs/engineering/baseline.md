...
| Web UI | Vue `3.5.34` + Vite `8.0.10` + Ant Design Vue `4.2.6` + Vue Vben Admin `5.7.0` 对齐方案 + Vue Router `5.0.7` + Pinia `3.0.4` |
...
| 运行环境资源准备 | `.deps/manifest.json` 可信来源测速 + `cache/downloads/runtime/` + `.deps/store/<resource-id>/<version>/` |
...
- 系统 `bash` 可能指向 WSL；Git Bash 使用 `C:\Program Files\Git\usr\bin\bash.exe`。
