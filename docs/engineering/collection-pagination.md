...
适配器状态是配置实例的完整快照，按配置顺序提供 HTTP 与 WS 状态；它不承担历史集合的增量遍历。服务状态是单对象，治理事件是失效通知，插件事件是单插件变化。日志与商店目录已有各自分页语义，保持其原有游标、过滤和上限。

数据库分页不等于插件内部读取 API 的改变：聊天策略仍查询完整有效治理规则，调度引擎仍装载所有需执行的作业，Catalog 与模板服务仍拥有完整运行状态。HTTP 查询不会删减这些权威数据。

分页回归覆盖超过两页的治理名单、后页插件的搜索/筛选、浏览器加载更多与条件切换，以及未加载插件/模板的详情定位。
