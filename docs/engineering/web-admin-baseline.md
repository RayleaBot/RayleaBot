...
- 协议中心、应用壳、认证入口、插件、治理、配置与诊断工作区统一使用产品组件。
...
- 插件列表、详情、商店、定时任务和导航复用 `PluginIcon`；缺图统一使用中性品牌标记，图标完整缩放，资源或刷新批次变化时重试。名称从当前插件快照解析，日志与诊断按需附带 ID；版本统一使用 `v` 前缀。商店的发布者、最新版本与安装包的作者、已安装版本各自标注。
- 插件状态事件同步更新摘要；完整刷新列表或包版本变化时使旧详情失效并重新读取，过期详情请求不得覆盖新快照。商店使用服务端分页游标加载后续条目，安装任务通过 `/api/system/tasks/{task_id}` 跟踪终态，再刷新目录和安装列表。
...
- 模板预览目录由当前有效源文件构建；数据库 `render_templates` 只缓存每个模板的当前内容，源文件移除或无效时清理缓存。名称直接取 `template.json` 必填的 `name`，插件名称来自插件目录；模板 ID 仅用于路由与诊断。预览只使用服务端返回的 HTML 与示例数据，`source_digest` 标识当前源码和资源内容，用于刷新预览资源缓存。目录刷新清理旧详情，过期请求不得恢复已移除或已更新的内容。
- 页签是否显示不改变 keep-alive、搜索跳转和工作区恢复语义；显示规则见 [Web 界面规范](../design/web-management-ui.md)。
