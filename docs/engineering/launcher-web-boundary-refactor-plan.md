...
| Launcher 手工维护契约同构类型 | 成立 | Launcher 从 `contracts/web-api.openapi.yaml` 生成类型，服务端契约同构结构只保留生成物 |
...
| Web 的 Launcher 登录链路属于不合理耦合 | 成立 | Launcher 打开 Web 管理面只传普通 URL，Web 会话由初始化和登录流程建立 |
...
- Renderer 与 Tray 只消费共享展示推导结果，服务状态名来自同一套推导函数。
...
- Launcher 打开 Web 管理面时只打开普通 URL。
- Web 会话通过初始化和登录流程建立。
- 未登录打开受保护深链时，Web 进入登录页；登录或初始化完成后回到目标页面。
...
- Launcher 本地检查只覆盖启动前必须确认的本地条件；`.deps`、运行时、模板和长路径等深层运行态告警由服务端诊断表达。
