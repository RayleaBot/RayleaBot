...
- SQLite 内的当前版本、历史 revision 与校验摘要继续作为运行时模板真相与缓存依据。

## 预览与任务链

- `render.preview` 继续使用现有任务模型和 `/ws/tasks`。
- 管理面预览只针对当前已存模板版本。
- 模板预览页在模板、主题、输出格式或输入数据变化后自动提交最新预览请求。
- 相同 payload 不重复提交，旧任务结果和旧 artifact 下载结果不会覆盖当前结果。
- 管理面可以在模板预览页查看任务状态、产物信息、图片结果和任务详情入口。
...
- 页面内显示模板列表、模板基础信息、输入结构、预览参数、预览结果和任务入口。
- 本地 JSON 解析错误和预览任务错误会在同一页面内直接展示。
