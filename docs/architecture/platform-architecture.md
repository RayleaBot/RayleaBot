...
| 服务组装 | App、Config、System Service、Protocol Service、Bilibili Source | 组装服务、加载配置、发布状态、协调启动与关闭 |
...
- 三方监控页消费 `/api/third-party/monitors`、`/api/third-party/media`、`/api/bilibili/source/status` 与 `/api/bilibili/source/restart`。
- `/ws/events` 通过 `source: bilibili` 的 `events.received` 分支推送 Bilibili source 状态摘要。
...
| `server/internal/integrations/{weibo,douyin,netease_music}` | 平台登录、资料解析、Cookie 校验和用户解析实现 |

新增三方平台只实现平台 provider、账号校验函数和用户解析函数；共享账号、二维码会话和受控 HTTP 请求能力由上述基础包提供。
...
| 对外对象 | 职责 |
| --- | --- |
| `Module.Source` | Bilibili 直播与动态事件源，暴露启动、状态、重启和监控快照能力 |
| `Module.AccountClient` | Bilibili CK 校验和账号资料读取 |
| `Module.HTTPCredentials` | Bilibili HTTP 请求的受控凭据注入 |
| `RuntimeEvent` | 将 Bilibili 领域事件投影为插件 runtime 事件 |

`bilibili/source` 只处理 Bilibili 直播、动态、状态持久化和诊断，不直接依赖插件 runtime 协议。app/servicegraph 通过 `bilibili.Build` 获取模块对象，不直接构造 source 内部仓储、session、diagnostics 或 monitoring 细节。
...
| Bilibili source | `server/internal/integrations/bilibili/source/` |
| Bilibili HTTP / session / monitoring | `server/internal/integrations/bilibili/` |
