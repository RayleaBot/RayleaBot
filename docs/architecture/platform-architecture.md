...
- **CLI 模式**：带子命令参数时复用同一二进制执行备份、恢复、迁移、依赖检查与诊断子命令，运行结束即退出。
...
- **Tasks 通道**：订阅插件安装、备份、恢复、迁移等异步任务的进度与终态。
- **Logs 通道**：推送结构化日志流并支持 spool 回放。

CLI 模式下 backup、restore、迁移、诊断子命令通过 `app` 包内组装的同一套服务对象访问 Storage、Secrets、Recovery 等子系统，使本地诊断与服务运行使用同一组真理来源。
...
| SQLite | 唯一正式状态库，保存 auth、tasks、plugins、grants、scheduler、logs；迁移文件嵌入二进制，查询经 sqlc 生成 |
| `data/` | 状态库与插件业务数据沙箱 |
...
| Storage(SQLite) | `server/internal/storage/`（嵌入 `migrations/`） |
