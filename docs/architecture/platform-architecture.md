...
内置三方功能由 Server 直接承载，当前正式平台为 Bilibili。

- Third-Party Account Service 管理 `/api/third-party/accounts`，账号摘要保存在 SQLite，CK 等敏感凭据保存在 secret store。
...
| Event Ingress | `server/internal/eventingress/` |
...
| 三方账号 | `server/internal/thirdparty/` |
...
- **插件 desired state 与授权**：`server/internal/plugins/repository/`、`server/internal/plugins/grants/`
