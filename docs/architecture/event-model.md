...
出站以被动回复为主：回复需引用收到的消息 id，并按 `msg_seq` 递增；主动推送有配额限制。回复目标记录事件来源协议，因此回复自动路由到对应适配器；主动推送可由插件在 `message.send` 的 `source_protocol` 中指名适配器；未指名时只在单适配器运行时可解析，多适配器下路由器拒绝并列出已连接的适配器，因为目标标识按协议各自命名，猜错可能发到另一个平台上恰好同标识的会话。
