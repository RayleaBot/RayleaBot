...
插件后端是经过 artifact 校验的当前平台原生可执行文件，运行在独立子进程中，通过 JSONL plugin protocol v2 与服务端通信。当前版本不提供插件 OS 强沙盒；管理员启用的插件按完全可信本地代码运行，但不能直接访问服务端内部对象。

## Artifact 边界

安装器只接受平台预编译目录或单根目录 ZIP。`info.json` v3 描述语言无关插件合同，`artifact.json` v2 固定目标平台、入口和精确文件摘要。完整性、最低 Core 版本、平台、二进制格式、路径边界和 UI 入口全部通过后，安装器才原子发布目录。
