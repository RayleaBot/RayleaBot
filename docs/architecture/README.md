...
5. Bridge 校验统一事件结构，补齐桥接层观测字段，并把事件交给 Dispatcher。
6. Dispatcher 只选择处于 `running` 的插件 runtime，按命令声明或事件订阅关系 fan-out 排队。
7. Runtime Manager 把事件投递给插件子进程，并接收结果、错误和本地 action。
8. Dispatcher 执行插件返回的出站动作；需要图片输出时，Bot Core 调用 Render Service 生成或复用缓存 artifact。
