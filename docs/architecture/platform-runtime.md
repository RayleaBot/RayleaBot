...
`scripts/release/rehearse_current_recovery.py` 使用真实 Server 在新建目录中初始化、创建管理员与插件业务数据，执行备份，再恢复到另一个空目录。`--legacy-schema` 构造真实 `000001` 归档，验证首次启动迁移；默认模式验证当前结构恢复。两种模式均核对配置、数据库、插件文件、恢复后的登录和重复启动结果，输出包含过程日志与结果 JSON。
