...
恢复包从归档内的配置文档与 SQLite 快照读取 schema 版本；无法确认时标为 `unknown`，不使用当前二进制版本代填。配置导入和恢复预检共用已实现的迁移路径，目前接受 v3 → v4 和当前 v4，其他已知版本须先手工转换。SQLite 启动时顺序执行编号迁移，不删除旧迁移脚本。

第 8 号数据库迁移把旧黑白名单合并为带作用域的条目，保留类型、目标、原因、创建时间和白名单开关。旧记录没有协议与 bot 信息，统一归入 OneBot 全局作用域；原先用来匹配 QQ 官方 openid 的条目需要管理员重新绑定实际实例及 bot。

旧 SHA-256 密码摘要在下一次成功登录时升级；离线工具没有用户密码，不能提前计算新摘要。配置 secret 的旧键只在已定义的 v3 迁移映射内搬运。这两条路径是既有数据的导入步骤，保留幂等处理以支持中断恢复。

`scripts/release/rehearse_data_migration.py` 使用合成的 v3 配置、v7 数据库和旧密码摘要，经过真实 CLI 备份、恢复、Server 启动及登录，验证迁移结果与进程退出。该演练不读取开发环境用户数据，也不替代签名发行包的跨版本恢复验证。
