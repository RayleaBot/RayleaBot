...
SQLite 从 `server/internal/storage/schema.sql` 在事务内一次创建当前结构，并在 `schema_metadata` 的唯一记录保存结构版本与初始化时间。重复启动读取该记录复用同一结构，不重建表或覆盖业务数据；有业务表但缺失或无效元数据、结构版本不符时明确报错。结构变更统一修改 `schema.sql` 并同步版本常量与 sqlc 生成物，应用查询明确列名。配置版本为 `4`，数据库结构版本为 `000001`，管理员密码使用带随机盐的 Argon2id 格式。
