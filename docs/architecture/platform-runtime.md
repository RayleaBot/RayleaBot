...
恢复包从归档内的配置文档与 SQLite 快照读取 schema 版本；无法确认时标为 `unknown`，不使用当前二进制版本代填。配置导入与恢复预检只接受当前配置格式。SQLite 启动时顺序执行编号迁移，不删除旧迁移脚本。
...
旧 SHA-256 密码摘要在下一次成功登录时升级；离线工具没有用户密码，不能提前计算新摘要。配置 secret 使用当前字段对应的存储键。

`scripts/release/rehearse_data_migration.py` 使用当前格式配置、v7 数据库和旧密码摘要，经过真实 CLI 备份、恢复、Server 启动及登录，验证迁移结果与进程退出。该演练不读取开发环境用户数据，也不替代签名发行包的跨版本恢复验证。
