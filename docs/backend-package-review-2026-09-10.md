...
内置菜单仅为 `RenderIdentityData` 导入 `plugins/actions`；Ingress 直接持有 `*lifecycle.Controller`，由此传递依赖进程 runtime。证据：[menu.go:17、406](../server/internal/builtinmenu/menu.go)、[render_identity.go:8–18](../server/internal/plugins/actions/render_identity.go)、[ingress.go:14、30](../server/internal/eventpipeline/chatpolicy/ingress.go)。
...
| 当前依赖 | 证据与影响 | 调整 |
| --- | --- | --- |
| `plugins → storage/sqlcgen` | [repository.go:9–18](../server/internal/plugins/repository.go)；根包被 16 个生产包直接消费，模型消费者同时依赖 SQLite 实现 | 根保留模型和仓储接口，SQLite 实现移到已存在的 `plugins/catalog/repository.go`；不新增只放一个实现的 state 包 |
