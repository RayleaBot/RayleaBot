...
| Web 管理面 | `pending-runtime` | 现有 Ant Design Vue tokens 与 CSS variables 继续生效；主题偏好只包含显式亮色和暗色 | 语义 token 完成映射，首次主题跟随系统，页面组合满足 Web 分面验收条件 |
| Launcher | `pending-runtime` | 已支持 `light`、`dark`、`system`；现有 Fluent theme 与 CSS variables 尚未映射项目级冷暖语义 | 颜色、层次、密度与组件状态完成映射，桌面壳职责保持不变 |
...
`documented` 表示正式文档与机器可读上下文完整，`pending-runtime` 表示运行时仍需采用，`compatible-envelope` 表示当前边界允许独立实现，`requires-contract-first` 表示实现前必须冻结正式契约。
