...
- 字段交互参考 Apple [Text fields](https://developer.apple.com/design/human-interface-guidelines/text-fields)，具体焦点样式由 Web 认证组件定义：`1px` 边框搭配紧贴边缘的 `3px` 柔和着色光环，不叠加分离的 `2px` 外轮廓。forced-colors 下使用 `2px` 系统焦点轮廓与 `2px` 间距；主按钮、主题切换和文字操作保留各自的键盘焦点轮廓。
...
- 常规焦点轮廓使用专用 `focus` token，为 `2px` 外轮廓与 `2px` 间距，轮廓与相邻背景至少达到 `3:1`；认证输入使用认证入口规定的贴边光环与 forced-colors 降级。
