...
- 品牌、页面与面板标题、普通正文、控件及插件卡片版本使用共享的自托管 Noto Sans SC。Web 的 `web/src/styles/_base.scss` 在 `:root` 将 `--font-sans` 局部映射到现有 `--font-display`，管理面和认证入口的 Ant Design theme 均消费 `var(--font-sans)`；字体子集与授权文件随 Web 构建发布。
...
- 展开侧栏的分类标题仅显示文字与展开箭头，行高 `24px`、上间距 `6px`、字号 `12px`、字重 `400`，使用现有中性辅助色并与页面项图标列对齐。分类标题无图标、底托、边框、圆角或阴影；分类标题始终使用透明背景；悬停时仅将文字和箭头提升为高对比中性前景，不添加底色或位移动效，展开或包含当前页面不单独提亮。
- 分类标题使用普通箭头光标，保留整行点击展开/收起、键盘导航与中性 `focus-visible` 轮廓，不套用普通导航项的悬停底色。移动抽屉使用相同弱化样式，可点击目标至少为 `44px`；普通页面项保留悬停反馈；当前页以中性底色和主题色文字表示选中，图标颜色保持中性；`64px` 收起侧栏图标模式遵循既有规则。
...
- 内容区不显示插件中心切换栏。面包屑、搜索和工作区页签继续使用原有深链；插件详情保留独立工作区页签及实际页面标题。侧栏优先打开插件现有工作区页签的完整 URL，没有对应页签时进入概览；折叠侧栏和移动端保留详情页内的分段控件。
- 菜单保存与未保存状态位于左侧字段下方；商店刷新、目录来源与条目数量位于筛选工具栏；指令权限入口位于指令筛选旁。插件列表安装入口及全局插件设置保存栏保持各自位置。必要的字段说明、业务状态、错误与空态信息保留。
- 记忆中的五个旧页签合并于该组最早位置，其他工作区保持顺序；当前子页 URL 优先，其次使用已有统一页签的有效返回地址，否则进入插件列表。相同配置快照的重复读取保留菜单中心和全局插件设置的本地草稿，实际配置更新与保存结果按原页面规则同步。
