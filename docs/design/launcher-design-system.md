...
主题入口使用显式菜单，按“跟随系统、浅色、深色”排列并显示当前单选项。菜单以 `220ms` 淡入和轻微位移打开，以 `160ms` 淡出和轻微缩放关闭；选中反馈在退出期间保持可见，弹层消失后主题颜色在 `220ms` 内平滑过渡，并把焦点还给触发按钮。`prefers-reduced-motion` 下立即完成开合与主题切换。
...
- 页面标题为 `24px`，关键状态与分区标题为 `18px`，组标题为 `16px`，正文为 `14px`，任务标签、路径和日志为 `13px`。`12px` 只用于窗口 chrome 与短状态标签。
